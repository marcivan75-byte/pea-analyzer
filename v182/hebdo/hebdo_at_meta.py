...
REQUIRED = {'ticker','close','vol_z','drawdown_4w','atr_14_pct','mom_26w_sector'}
...
class HebdoATMeta:
    ...
    def run(self, df_features: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df_features.copy().drop_duplicates('ticker', keep='last')
        n0 = len(df)
...
def main():
    ...
    if not src.exists() or src.stat().st_size == 0:
        raise SystemExit(f"BLOCK_DATA_META: missing features {src}")
    df=pd.read_csv(src)
    ranked=HebdoATMeta().run(df)
    ranked.to_csv(out/'HEBDO_AT_META_RANKED.csv', index=False)
    summary={
        'status':'OK', 'version':'HEBDO_AT_META_V1',
        'universe_initial':int(ranked['meta_universe_initial'].iloc[0]) if len(ranked) else 0,
        'universe_after_fp':int(len(ranked)),
        'TCT':int((ranked['tier']=='TCT').sum()),
        'CT_WATCH':int((ranked['tier']=='CT_WATCH').sum()),
        'EXCLU':int((ranked['tier']=='EXCLU').sum()),
        'meta_model_status': str(ranked['meta_model_status'].iloc[0]) if 'meta_model_status' in ranked and len(ranked) else 'UNAVAILABLE',
