...
class HebdoATMeta:
    ...
    def _normalize_universe(self,df:pd.DataFrame):
        ...
        if out['ticker'].duplicated(keep=False).any():
            if 'date' not in out.columns: raise ValueError('BLOCK_DATA_META: duplicate tickers require date for chronological resolution')
            out['_meta_row_date']=pd.to_datetime(out['date'],errors='coerce',utc=True); dup_mask=out['ticker'].duplicated(keep=False)
            if out.loc[dup_mask,'_meta_row_date'].isna().any(): raise ValueError('BLOCK_DATA_META: duplicate ticker has invalid date')
            if out.loc[dup_mask,['ticker','_meta_row_date']].duplicated().any(): raise ValueError('BLOCK_DATA_META: duplicate ticker rows share same timestamp')
            before=len(out); out=out.sort_values(['ticker','_meta_row_date']).drop_duplicates('ticker',keep='last'); duplicate_rows=before-len(out); out=out.drop(columns=['_meta_row_date'])
        return out,rows_input,duplicate_rows

    def run(self,df_features:pd.DataFrame)->pd.DataFrame:
        self.validate(df_features); df,rows_input,n_duplicate=self._normalize_universe(df_features); n0=len(df)
        ...
        numeric=df[CRITICAL_NUMERIC].to_numpy(dtype=float); invalid=~np.isfinite(numeric).all(axis=1)
        ...
        df['META_STATUS']=df['tier']; df['meta_rows_input']=rows_input; df['meta_universe_initial']=n0; df['meta_duplicate_rows_resolved']=n_duplicate
        df['meta_invalid_rows_dropped']=n_invalid; df['meta_universe_after_fp']=n_after_fp; df['process_stage']='RANKED'
        return df
    ...
    def run_preopen(self,ranked:pd.DataFrame,as_of_date:str)->pd.DataFrame:
        ...
        out['preopen_actionable']=out['preopen_data_status'].eq('OK'); out['process_stage']=np.where(out['preopen_actionable'],'PREOPEN_ENRICHED','PREOPEN_BLOCKED')
        return out
    ...
    def run_confirmation_j1(self,candidates:pd.DataFrame,next_bars:pd.DataFrame)->pd.DataFrame:
        ...
        if 'preopen_data_status' not in candidates.columns: raise ValueError('BLOCK_DATA_CONFIRMATION: preopen stage evidence missing')
        out=candidates.copy(); blocked=~out['preopen_data_status'].eq('OK'); actionable=out.loc[~blocked].copy(); blocked_rows=out.loc[blocked].copy()
        if not blocked_rows.empty:
            blocked_rows['enter_confirmed']=None; blocked_rows['confirm_reason']='BLOCK_PREOPEN_NOT_ACTIONABLE'; blocked_rows['confirmation_status']='BLOCK'; blocked_rows['confirmation_actionable']=False; blocked_rows['process_stage']='J1_BLOCKED_PREOPEN'
...
def main():
    ...
    if not src.exists() or src.stat().st_size==0: _write_block(out,f'missing features {src}'); raise SystemExit(2)
    try:
        provenance=PITLoader(root=Path('.'),strict_provenance=True).validate_explicit_provenance(src,pd.Timestamp(args.as_of)); df=pd.read_csv(src); ranked=HebdoATMeta().run(df)
    ...
    summary={'status':'OK','version':'HEBDO_AT_META_V1','as_of':str(args.as_of),'pit_snapshot_time':provenance['snapshot_time'],'pit_cutoff':provenance['cutoff'],'pit_provenance_source':provenance['source'],
