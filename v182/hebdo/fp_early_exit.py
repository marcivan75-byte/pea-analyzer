...
class FPEarlyExit:
    ...
    def check_exit(self, entry_price: float, current_bar: dict, days_held: int, sector_bar: dict = None) -> Tuple[bool, str, Optional[float]]:
        ...
        if vol_z is not None and math.isfinite(vol_z) and vol_z>5 and close<entry_price:
        ...
        # Le trailing +1% n'est actif que si +3% a été atteint AVANT la barre courante.
        ...
        if days_held>=5 and peak_prior is not None and math.isfinite(peak_prior) and peak_prior>=0.03 and pnl_low<=0.01:
