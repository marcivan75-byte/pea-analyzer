...
HEBDO AT META - confirmation J+1 avec preuve chronologique obligatoire et journal complet des décisions.
"""
...
class ConfirmationEntry:
    def __init__(self,max_calendar_gap_days:int=5):
        self.max_gap_down=-0.01; self.min_confirm=0.005; self.max_calendar_gap_days=int(max_calendar_gap_days)
        if self.max_calendar_gap_days<1: raise ValueError('BLOCK_DATA_CONFIRMATION: max_calendar_gap_days must be >=1')
    ...
    def should_enter(self,row_signal:pd.Series,bar_next:pd.Series)->Tuple[Optional[bool],str]:
        ...
        if any(pd.isna(x) for x in [close_prev,open_next,close_next]) or close_prev<=0 or open_next<=0 or close_next<=0: return None,'BLOCK_DATA_NEXT_BAR'
    ...
    def _next_bar_for_row(self,row:pd.Series,bars:pd.DataFrame)->Tuple[pd.Series,Optional[str]]:
        ticker=row.get('ticker'); subset=bars[bars['ticker'].eq(ticker)].copy()
        if subset.empty: return pd.Series(dtype=float),'BLOCK_DATA_NEXT_BAR'
        if 'date' not in row.index or 'date' not in subset.columns or pd.isna(row.get('date')): return pd.Series(dtype=float),'BLOCK_DATA_NEXT_BAR_DATE_REQUIRED'
        ...
        if subset['_date'].isna().any(): return pd.Series(dtype=float),'BLOCK_DATA_NEXT_BAR_INVALID_DATE'
        subset=subset[subset['_date']>signal_date].sort_values('_date')
        if subset.empty: return pd.Series(dtype=float),'BLOCK_DATA_NEXT_BAR'
        if subset['_date'].duplicated().any(): return pd.Series(dtype=float),'BLOCK_DATA_NEXT_BAR_DUPLICATE_TIMESTAMP'
        next_row=subset.iloc[0]; gap_days=(next_row['_date']-signal_date).total_seconds()/86400
        if gap_days>self.max_calendar_gap_days: return pd.Series(dtype=float),f'BLOCK_DATA_NEXT_BAR_STALE_{gap_days:.1f}d'
    ...
    def filter_batch_j1(self,df_friday:pd.DataFrame,df_next_bars:pd.DataFrame):
        ...
        if 'ticker' not in df.columns or 'ticker' not in df_next_bars.columns:
            df['enter_confirmed']=None; df['confirm_reason']='BLOCK_DATA_NEXT_BAR'; df['confirmation_status']='BLOCK'; return df
        results=[]
        ...
        def status(v,reason):
            if v is True: return 'CONFIRMED'
            if v is False: return 'REJECT'
            return 'BLOCK' if str(reason).startswith('BLOCK') else 'WAIT'
        df['confirmation_status']=[status(v,r) for v,r in results]
        df['confirmation_actionable']=df['confirmation_status'].eq('CONFIRMED')
        return df
