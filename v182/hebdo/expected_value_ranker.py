...
class ExpectedValueRanker:
    def __init__(self, avg_win=0.14, avg_loss=-0.09, fee=0.003, parameter_status='PARAMETRIC_UNCALIBRATED'):
        ...
        if self.avg_win<=0 or self.avg_loss>=0 or self.fee<0:
    ...
    def compute_ev(self, row: pd.Series)->float:
        ...
        malus=0.0
        mom=row.get('mom_26w_sector',0); mom=0 if pd.isna(mom) else float(mom)
        mult=0.5 if mom>1.0 else 1.0
        roe=row.get('roe',np.nan); dd=row.get('drawdown_4w',0); vol_z=row.get('vol_z',0); dte=row.get('days_to_earnings',np.nan)
    ...
    def rank_batch(self, df: pd.DataFrame):
        ...
        if not np.isfinite(df['EV_net'].to_numpy(dtype=float)).all():
            raise ValueError('BLOCK_DATA_EV: non-finite EV')
        if len(df)>=20:
            q85=df['EV_net'].quantile(0.85); q60=df['EV_net'].quantile(0.60)
        else:
            q85=df['EV_net'].median(); q60=df['EV_net'].quantile(0.4)
        df['tier']='EXCLU'
        tct_floor=max(float(q85),0.0); ct_floor=max(float(q60),0.0)
