...
def score_queries(
    ...
    *,
    ...
    """Score each exact unique GDELT query once with bounded concurrent I/O.

    Logical consumers may request the same query multiple times; only the exact
    query/timespan/max_records tuple is deduplicated. Request starts remain
    globally rate-limited while network waits overlap.
    """
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            out[query]=future.result()
    # Populate the exact cache after the batch so any later same-run direct call
    # returns the identical result without a second network request.
    for query,(score,error) in out.items():
        # lru_cache has no public insertion API; later top-down consumers use out.
        # Keep score_query for other call sites unchanged and deterministic.
        _ = (score,error)
    return out
