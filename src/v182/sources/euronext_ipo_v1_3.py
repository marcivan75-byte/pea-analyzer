...
def parse_ipo_price(value: object) -> tuple[float | None, str]:
    ...
    # IPO-price strings can contain descriptive text; the last numeric token is generally the quoted price.
    try:
...
def collect_euronext_v1_3(start: date, end: date, timeout: int = 20) -> tuple[list[dict], dict]:
    ...
    This source remains best-effort. Missing detail pages never invent evidence and never
    fail the complete IPO radar when the base Euronext calendar can still be parsed.
    """
    source = "EURONEXT"
