 ...
 for trail in [None,5,7]:
  for heat in ['none','rsi','stoch','both']:
   for trend in ['none','psar','sma20','psar_sma20']:
    if hard is None and trail is None and heat=='none' and trend=='none': continue
...
def simulate(sym,b,sig,entry_label,cfg,cfg_id):
 pos=None; out=[]
 opens=b.open.to_numpy(float); closes=b.close.to_numpy(float)
 for i in range(1,len(b)-1):
  nxt_open=opens[i+1]
  if pos is not None and i>=pos['idx']:
   reasons,peak=exit_reasons(b,i,pos,cfg); pos['peak_close']=peak
   if reasons and np.isfinite(nxt_open) and nxt_open>0:
    out.append({'symbol':sym,'entry_model':entry_label,'exit_model':cfg_id,'entry_date':b.index[pos['idx']].date().isoformat(),'exit_date':b.index[i+1].date().isoformat(),'return_pct':(nxt_open/pos['price']-1)*100,'holding_weeks':i+1-pos['idx'],'exit_reasons':'|'.join(reasons)}); pos=None; continue
  if pos is None and bool(sig[i]) and np.isfinite(nxt_open) and nxt_open>0:
   pos={'idx':i+1,'price':nxt_open,'peak_close':nxt_open}
 return out
...
def metric(df):
 ...
 pf=float(win.sum()/(-loss.sum())) if len(loss) and -loss.sum()>0 else 99.0; rr=float(aw/abs(al)) if al<0 else 99.0
...
def summarize(df,end):
 res={'ALL':metric(df)}; d=pd.to_datetime(df.entry_date,errors='coerce') if not df.empty else pd.Series(dtype='datetime64[ns]')
 ...
 robust_rr=min((x['reward_risk'] or 0) for x in vals) if vals else 0; robust_pf=min((x['profit_factor'] or 0) for x in vals) if vals else 0
 robust_mean=min((x['mean_return_pct'] if x['mean_return_pct'] is not None else -999) for x in vals) if vals else -999; worst_p10=min((x['p10_return_pct'] if x['p10_return_pct'] is not None else -999) for x in vals) if vals else -999
...
def run():
 st=time.perf_counter(); OUT.parent.mkdir(parents=True,exist_ok=True); bars={}; signals={}; first=[]; last=[]
 ...
 for ci,cfg in enumerate(CONFIGS,1):
  ...
  for ent in ENTRIES:
   tr=[]
   for sym,b in bars.items(): tr.extend(simulate(sym,b,signals[sym][ent['label']],ent['label'],cfg,cfg_id))
   df=pd.DataFrame(tr); by_entry[ent['label']]=summarize(df,end); all_tr.extend(tr)
  combined=summarize(pd.DataFrame(all_tr),end)
  eligible=all(by_entry[e['label']]['ALL']['trades']>=50 for e in ENTRIES)
  weak_rr=min(by_entry[e['label']]['selection_key'][0] for e in ENTRIES); weak_pf=min(by_entry[e['label']]['selection_key'][1] for e in ENTRIES)
