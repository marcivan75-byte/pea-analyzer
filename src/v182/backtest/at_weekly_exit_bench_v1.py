 ...
 for i in range(1,len(c)-1):
  ...
  if pos_idx>=0 and i>=pos_idx:
   ...
   if cfg['baseline']:
    ...
    heat=cfg['heat']
    if heat in {'rsi','both'} and np.isfinite(rsi[i]) and rsi[i]>75 and close<prev: reasons.append('RSI_GT75_REV')
    crossdn=np.isfinite(k[i]) and np.isfinite(d[i]) and np.isfinite(k[i-1]) and np.isfinite(d[i-1]) and k[i]<d[i] and k[i-1]>=d[i-1]
    if heat in {'stoch','both'} and np.isfinite(k[i]) and k[i]>75 and crossdn: reasons.append('STOCH_GT75_CROSSDOWN')
    tr=cfg['trend']
...
def summarize(df,end):
 ...
 rr=min((x['reward_risk'] or 0) for x in vals) if vals else 0; pf=min((x['profit_factor'] or 0) for x in vals) if vals else 0
 ...
 a=res['ALL']; res['selection_key']=[round(rr,4),round(pf,4),round(p10,4),round(mean,4),a['win_rate_pct'] or 0,min(a['trades'],250)]
 return res
...
def run():
 ...
 for ci,cfg in enumerate(CONFIGS,1):
  ...
  comb=summarize(pd.DataFrame(alltr),end); eligible=all(by[e['label']]['ALL']['trades']>=50 for e in ENTRIES)
