"""Compare AT weekly baseline vs entry-only close-location filters for ACTIONS.
...
OUT = ROOT / "outputs/backtest/AT_WEEKLY_CLOSE_LOCATION_FILTER_V1.json"
OUT_CSV = ROOT / "outputs/backtest/AT_WEEKLY_CLOSE_LOCATION_FILTER_V1_TRADES.csv"
...
def backtest(symbol, weekly, variant):
    ...
    bars['close_location']=(bars['close']-bars['low'])/rng
    bars['ma20_gt_ma50']=bars['sma20']>bars['sma50']
    base=bars['entry_signal'].fillna(False)
    if variant=='BASELINE': entry=base
    elif variant=='CLOSE_LOC_GE_060': entry=base & (bars['close_location']>=0.60)
    elif variant=='MA20_GT_MA50': entry=base & bars['ma20_gt_ma50'].fillna(False)
    elif variant=='MA20_GT_MA50_CLOSE_LOC_GE_060': entry=base & bars['ma20_gt_ma50'].fillna(False) & (bars['close_location']>=0.60)
...
def run():
    started=time.perf_counter(); variants=['BASELINE','CLOSE_LOC_GE_060','MA20_GT_MA50','MA20_GT_MA50_CLOSE_LOC_GE_060']
    ...
    payload={"status":"SUCCESS","version":"AT_WEEKLY_CLOSE_LOCATION_FILTER_V1","generated_at_utc":datetime.now(timezone.utc).isoformat(),"runtime_seconds":round(time.perf_counter()-started,3),"valid_actions":valid,"data_window":{"first_week":min(first).date().isoformat() if first else None,"last_week":max(last).date().isoformat() if last else None},"exit_rules_changed":False,"entry_variants":{"BASELINE":"AT V1.1","CLOSE_LOC_GE_060":"baseline + close location >= 0.60","MA20_GT_MA50":"baseline + SMA20 > SMA50","MA20_GT_MA50_CLOSE_LOC_GE_060":"baseline + SMA20 > SMA50 + close location >= 0.60"},"results":results,"limitations":["CURRENT_CACHE_UNIVERSE_NOT_PIT_MEMBERSHIP","SURVIVORSHIP_BIAS_POSSIBLE","NO_FEES_SLIPPAGE","RESEARCH_ONLY"]}
