"""Compare AT weekly baseline vs entry-only volume filter for ACTIONS.
...
OUT = ROOT / "outputs/backtest/AT_WEEKLY_ENTRY_FILTERS_V1.json"
OUT_CSV = ROOT / "outputs/backtest/AT_WEEKLY_ENTRY_FILTERS_V1_TRADES.csv"


def metrics(df: pd.DataFrame):
    if df.empty:
        return {"trades":0,"win_rate_pct":None,"mean_return_pct":None,"median_return_pct":None,"profit_factor":None,"one_week_trades":0,"one_week_win_rate_pct":None,"one_week_mean_return_pct":None}
...
def backtest(symbol, weekly, variant):
    ...
    if 'volume' in bars.columns:
        bars['vol_ma20']=bars['volume'].rolling(20,min_periods=20).mean()
        bars['vol_up']=bars['volume']>bars['volume'].shift(1)
        bars['vol_above_ma20']=bars['volume']>bars['vol_ma20']
    else:
        bars['vol_up']=False; bars['vol_above_ma20']=False
    base=bars['entry_signal'].fillna(False)
    if variant=='BASELINE': entry=base
    elif variant=='VOL_UP': entry=base & bars['vol_up'].fillna(False)
    elif variant=='VOL_GT_MA20': entry=base & bars['vol_above_ma20'].fillna(False)
    elif variant=='VOL_DOUBLE': entry=base & bars['vol_up'].fillna(False) & bars['vol_above_ma20'].fillna(False)
...
def run():
    started=time.perf_counter(); variants=['BASELINE','VOL_UP','VOL_GT_MA20','VOL_DOUBLE']
    trades=[]; counts=Counter(); valid=0; no_volume=0; first=[]; last=[]
    for path in _cache_files(ROOT/CACHE_DIRS['ACTION']):
        for symbol,hist,error in _iter_consolidated(path):
            ...
            if 'volume' not in weekly.columns or weekly['volume'].isna().all(): no_volume+=1
            for v in variants:
    ...
    payload={"status":"SUCCESS","version":"AT_WEEKLY_ENTRY_FILTERS_V1","generated_at_utc":datetime.now(timezone.utc).isoformat(),"runtime_seconds":round(time.perf_counter()-started,3),"valid_actions":valid,"actions_without_volume":no_volume,"data_window":{"first_week":min(first).date().isoformat() if first else None,"last_week":max(last).date().isoformat() if last else None},"exit_rules_changed":False,"entry_variants":{"BASELINE":"AT V1.1","VOL_UP":"baseline + volume N > N-1","VOL_GT_MA20":"baseline + volume N > moyenne 20 semaines","VOL_DOUBLE":"baseline + volume N > N-1 ET > moyenne 20 semaines"},"results":results,"limitations":["CURRENT_CACHE_UNIVERSE_NOT_PIT_MEMBERSHIP","SURVIVORSHIP_BIAS_POSSIBLE","NO_FEES_SLIPPAGE"]}
