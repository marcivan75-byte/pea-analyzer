"""Compare AT weekly baseline vs entry-only weekly range filters for ACTIONS.
...
OUT = ROOT / "outputs/backtest/AT_WEEKLY_RANGE_FILTER_V1.json"
OUT_CSV = ROOT / "outputs/backtest/AT_WEEKLY_RANGE_FILTER_V1_TRADES.csv"
...
def metrics(df: pd.DataFrame):
    ...
    return {"trades":int(len(r)),"win_rate_pct":round(float((r>0).mean()*100),2),"mean_return_pct":round(float(r.mean()),3),"median_return_pct":round(float(r.median()),3),"profit_factor":round(pf,3) if pf is not None and math.isfinite(pf) else None,"one_week_trades":int(len(orr)),"one_week_win_rate_pct":round(float((orr>0).mean()*100),2) if len(orr) else None,"one_week_mean_return_pct":round(float(orr.mean()),3) if len(orr) else None,"mean_holding_weeks":round(float(pd.to_numeric(df.holding_weeks,errors='coerce').mean()),2)}
...
def backtest(symbol, weekly, variant):
    ...
    bars['close_location']=(bars['close']-bars['low'])/rng.replace(0,np.nan)
    bars['range_pct']=100*rng/bars['close'].replace(0,np.nan)
    base=bars['entry_signal'].fillna(False)
    best=base & bars['ma20_gt_ma50'].fillna(False) & (bars['close_location']>=0.60)
    if variant=='BASELINE': entry=base
    elif variant=='BEST_CURRENT': entry=best
    elif variant=='RANGE_LE_8': entry=base & (bars['range_pct']<=8)
    elif variant=='RANGE_LE_10': entry=base & (bars['range_pct']<=10)
    elif variant=='BEST_PLUS_RANGE_LE_8': entry=best & (bars['range_pct']<=8)
    elif variant=='BEST_PLUS_RANGE_LE_10': entry=best & (bars['range_pct']<=10)
...
def run():
    started=time.perf_counter(); variants=['BASELINE','BEST_CURRENT','RANGE_LE_8','RANGE_LE_10','BEST_PLUS_RANGE_LE_8','BEST_PLUS_RANGE_LE_10']
    ...
    for path in _cache_files(ROOT/CACHE_DIRS['ACTION']):
        for symbol,hist,error in _iter_consolidated(path):
            ...
            for v in variants:
                t,b,s=backtest(symbol,weekly,v); trades.extend(t); counts[f'{v}_selected_signals']+=s
    ...
    payload={"status":"SUCCESS","version":"AT_WEEKLY_RANGE_FILTER_V1","generated_at_utc":datetime.now(timezone.utc).isoformat(),"runtime_seconds":round(time.perf_counter()-started,3),"valid_actions":valid,"data_window":{"first_week":min(first).date().isoformat() if first else None,"last_week":max(last).date().isoformat() if last else None},"exit_rules_changed":False,"entry_variants":{"BASELINE":"AT V1.1","BEST_CURRENT":"baseline + SMA20>SMA50 + close location>=0.60","RANGE_LE_8":"baseline + weekly range<=8%","RANGE_LE_10":"baseline + weekly range<=10%","BEST_PLUS_RANGE_LE_8":"best current + weekly range<=8%","BEST_PLUS_RANGE_LE_10":"best current + weekly range<=10%"},"results":results,"limitations":["CURRENT_CACHE_UNIVERSE_NOT_PIT_MEMBERSHIP","SURVIVORSHIP_BIAS_POSSIBLE","NO_FEES_SLIPPAGE","RESEARCH_ONLY"]}
