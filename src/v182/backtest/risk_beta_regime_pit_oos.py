...
def _period_evaluation(frame: pd.DataFrame, period: str, gates: dict) -> dict:
    ...
    checks = {
        ...
        "positive_adjusted_expectancy": adjusted.get("expectancy") is not None and adjusted["expectancy"] > 0,
        "p05_tail_not_worse": adjusted.get("p05") is not None and baseline.get("p05") is not None and adjusted["p05"] >= baseline["p05"],
        "sequence_max_drawdown_not_worse": adjusted.get("sequence_max_drawdown") is not None and baseline.get("sequence_max_drawdown") is not None and adjusted["sequence_max_drawdown"] >= baseline["sequence_max_drawdown"],
    }
    return {
        "period": period,
        "baseline": baseline,
...
def run(root: Path = ROOT) -> dict:
    ...
    promoted = trigger_gate and all(item["pass"] for item in evaluations)
