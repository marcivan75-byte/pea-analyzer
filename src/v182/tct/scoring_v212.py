...
def apply_decisions(df,cfg):
    out=df.copy(); th=cfg['thresholds']; cv=cfg['coverage']; g=cfg['gates']; score=_num(out,'tct_score_v212'); gcov=_num(out,'tct_score_coverage_v212'); tcov=_num(out,'tct_technical_impulse_coverage_v212'); vcov=_num(out,'tct_volume_flow_squeeze_coverage_v212'); ccov=_num(out,'tct_catalyst_event_coverage_v212'); tech=_num(out,'tct_technical_impulse_score_v212'); vol=_num(out,'tct_volume_flow_squeeze_score_v212'); cat=_num(out,'tct_catalyst_event_score_v212'); ident=_num(out,'v182_ticker_validation_confidence_pct'); ident=ident.where(ident<=1,ident/100); liq=_num(out,'liquidity_percentile','iquidity_percentile'); liq=liq.where(liq<=1,liq/100); prob=_num(out,'tct_probability_20d_calibrated_v212'); gap=_num(out,'gap_up_pct_v212'); pea=out.get('pea_confidence',pd.Series('',index=out.index)).astype(str).str.upper().str.startswith('HIGH'); warn=out.get('profit_warning_flag',pd.Series(False,index=out.index)).astype(str).str.lower().isin({'true','1','yes','oui'}); grade=out.get('coverage_grade_v21',pd.Series('',index=out.index)).astype(str).str.upper(); decisions=[]; reasons=[]
    for i in out.index:
