...
def weighted_raw_score(
    ...
    """Aggregate GROK 0-100 PIT criterion scores with the cloned MT weights."""
    missing = set(backtested_weights) - set(criterion_scores)
    if missing:
        raise ValueError(f"missing required GROK criteria: {sorted(missing)}")
...
def exit_decision(position: PositionState) -> ExitDecision:
    """Historical replay policy cloned from ETF MT; not an operational exit authority."""
