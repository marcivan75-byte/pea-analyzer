...
def _get_info_with_backoff(ticker, stats: dict):
    ...
    for attempt in range(len(RATE_LIMIT_BACKOFF_SECONDS) + 1):
        ...
        except Exception as exc:  # network/provider dependent
            last_exc = exc
...
def _coverage(df: pd.DataFrame, fields: list[str]) -> dict[str, float]:
    result = {}
    for field in fields:
        result[field] = round(float((~_missing(df, field)).mean() * 100.0), 2)
    return result
...
def main() -> None:
    ...
    if len(df) != 1429 or df["isin"].astype(str).nunique() != 1429:
        raise RuntimeError("Yahoo backfill requires the canonical 1429 Actions universe")
    ...
    mappings = {
        "marketCap": "market_cap_v21",
        "enterpriseValue": "enterprise_value_v21",
        "forwardPE": "per_forward_v21",
        "trailingPE": "per_ttm_v21",
        "priceToBook": "pb_v21",
        "freeCashflow": "free_cash_flow_v21",
        "operatingCashflow": "operating_cash_flow_v21",
        "totalDebt": "total_debt_v21",
        "totalCash": "total_cash_v21",
        "ebitda": "ebitda_v21",
        "debtToEquity": "debt_to_equity_v21",
        "currentRatio": "current_ratio_v21",
        "beta": "beta_v21",
        "targetMeanPrice": "target_mean_v21",
        "targetMedianPrice": "target_median_v21",
        "targetLowPrice": "target_low_v21",
        "targetHighPrice": "target_high_v21",
        "numberOfAnalystOpinions": "n_analysts_v21",
        "shortRatio": "short_ratio",
        "forwardEps": "forward_eps_v21",
        "trailingEps": "trailing_eps_v21",
        "pegRatio": "peg_ratio_v21",
        "revenuePerShare": "revenue_per_share_v21",
    }
    pct_mappings = {
        "returnOnEquity": "roe_v21_pct",
        "returnOnAssets": "roa_v21_pct",
        "operatingMargins": "operating_margin_v21_pct",
        "profitMargins": "net_margin_v21_pct",
        "grossMargins": "gross_margin_v21_pct",
        "revenueGrowth": "revenue_growth_v21_pct",
        "earningsGrowth": "earnings_growth_v21_pct",
        "earningsQuarterlyGrowth": "earnings_quarterly_growth_v21_pct",
        "dividendYield": "dividend_yield_v21_pct",
        "payoutRatio": "payout_ratio_v21_pct",
    ...
    for i in candidates:
        ...
        try:
            ...
            # History already matched the ticker. If Yahoo exposes a price now, it must still be consistent.
            ...
            for src, dst in mappings.items():
                if _fill_missing(df, i, dst, _f(info.get(src))):
                    filled += 1
            for src, dst in pct_mappings.items():
                if _fill_missing(df, i, dst, _pct(info.get(src))):
                    filled += 1

            rec = _f(info.get("recommendationMean"))
            if rec is not None:
                score100 = max(0.0, min(100.0, (5.0 - rec) / 4.0 * 100.0))
                if _fill_missing(df, i, "consensus_score_100_v21", score100):
                    filled += 1
            label = info.get("recommendationKey")
            if label and _fill_missing(df, i, "consensus_label_v21", str(label).upper()):
                filled += 1
            ...
            if mc and fcf is not None and _fill_missing(df, i, "fcf_yield_v21", fcf / mc * 100.0):
                filled += 1
            if debt is not None and ebitda and _fill_missing(df, i, "debt_to_ebitda_v21", debt / ebitda):
                filled += 1
        ...
        except Exception as exc:  # provider/network dependent; backfill must remain non-destructive
            stats["provider_errors"] += 1
            errors.append({"ticker": ticker_name, "reason": f"{type(exc).__name__}: {str(exc)[:160]}"})
            time.sleep(NORMAL_DELAY_SECONDS)

    # Recompute derived fields after gap filling.
    ...
    audit = {
        ...
        **stats,
        ...
        "missing_data_policy": "FILL_GAPS_ONLY_NO_OVERWRITE",
        "identity_policy": "HISTORY_VALIDATED_AND_CURRENT_PRICE_MUST_NOT_CONTRADICT",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    }
    AUDIT.write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
    print("V21_ACTIONS_YAHOO_BACKFILL_OK", json.dumps(audit, ensure_ascii=False))
