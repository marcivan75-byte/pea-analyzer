...
COMMITTEE = ROOT / 'outputs/V21.0_ACTIONS_PEA_1829_COMMITTEE.csv'
...
def main() -> None:
    ...
    if len(df) != expected or df['isin'].astype(str).nunique() != expected:
        raise RuntimeError(f'V21 data-quality gate requires canonical {expected} committee')
    ...
    audit['data_quality_gate'] = {
        'applied': True, 'expected_rows': expected,
        ...
        'no_score_change': True, 'no_weight_change': True,
        'generated_at_utc': datetime.now(timezone.utc).isoformat(),
    }
    audit['decisions_after_data_quality_gate'] = {hz: df[f'decision_{hz}'].astype(str).value_counts().to_dict() for hz in ['ct','mt','lt','short']}
    audit['selection_after_data_quality_gate'] = {hz: int(_truthy(df[f'selection_{hz}']).sum()) for hz in ['ct','mt','lt','short']}
    AUDIT.write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding='utf-8')

    with SUMMARY.open('a', encoding='utf-8') as fh:
        fh.write('\n\n## Data-quality decision gate\n')
        fh.write(f"- Universe: **{expected} Actions PEA**\n")
    ...
    print('V21_ACTIONS_DATA_QUALITY_GATE_1829_OK', {'process_status': cert.get('process_validation_status'), 'downgrades': downgrades})
