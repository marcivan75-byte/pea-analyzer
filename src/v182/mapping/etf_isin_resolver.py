...
EXCHANGE_TO_YAHOO_SUFFIX = {
    ...
    "GY": "DE",   # Xetra (variante de code)
    "SW": "SW",   # SIX Swiss Exchange
    "VI": "VI",   # Wiener Boerse
    "IR": "IR",   # Euronext Dublin
...
# Ordre de préférence quand un ISIN est coté sur plusieurs places (on
# privilégie les places Euronext les plus courantes pour un PEA).
...
def resolve_isins(isins: list[str], api_key: str | None = None,
    ...
    """Interroge OpenFIGI par lots et renvoie, pour chaque ISIN, la liste des
    correspondances trouvées (une par place de cotation). Sans clé API, le
    débit est volontairement prudent (lots de 10, pause de 6s) : ajustez ces
    valeurs selon les limites affichées par l'API si vous avez une clé."""
    import requests
...
def pick_best_match(matches: list[dict]) -> dict | None:
    """Choisit la meilleure correspondance selon EXCHANGE_PREFERENCE, en ne
    gardant que les places pour lesquelles on connaît un suffixe Yahoo."""
...
def build_etf_ticker_map(etf_master_path: str | Path, output_map_path: str | Path,
    ...
    """Construit config/V18.2_ETF_TICKER_MAP.csv à partir du référentiel ETF.
    Les ISIN déjà présents dans le fichier de mapping existant ne sont pas
    requêtés à nouveau (politique 'no_repeat_api_request_policy')."""
    import pandas as pd
    ...
    resolved = resolve_isins(to_resolve, api_key=api_key)
    ...
    for isin in to_resolve:
        ...
        if best is None:
            gaps.append({"isin": isin, "reason": "NO_OPENFIGI_MATCH_ON_KNOWN_EXCHANGE",
                         "raw_matches": len(matches)})
            continue
        suffix = EXCHANGE_TO_YAHOO_SUFFIX[best["exchCode"]]
        yahoo_ticker = f"{best['ticker']}.{suffix}"
        new_rows.append({"isin": isin, "yahoo_ticker": yahoo_ticker})
...
if __name__ == "__main__":
    ...
    print(f"OpenFIGI — {summary['resolved']}/{summary['requested']} ISIN résolus, "
          f"{summary['gaps']} restent en gap (voir outputs/gaps).")
    sys.exit(0)
