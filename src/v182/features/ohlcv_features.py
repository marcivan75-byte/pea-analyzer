...
def calculate(frame: pd.DataFrame) -> dict:
    ...
    result["macd_hist_change_3d"] = float(macd_hist.iloc[-1] - macd_hist.iloc[-4]) if len(macd_hist) >= 4 and pd.notna(macd_hist.iloc[-4]) else None

    previous_close = close.shift(1)
    true_range = pd.concat([high - low, (high - previous_close).abs(), (low - previous_close).abs()], axis=1).max(axis=1)
    atr = true_range.rolling(14).mean()
    result["atr14"] = float(atr.iloc[-1]) if len(close) >= 15 and pd.notna(atr.iloc[-1]) else None
    result["atr14_pct"] = float(atr.iloc[-1] / close.iloc[-1] * 100.0) if result["atr14"] is not None and close.iloc[-1] else None
    result["opening_gap_pct"] = float((open_.iloc[-1] / previous_close.iloc[-1] - 1.0) * 100.0) if len(close) >= 2 and pd.notna(open_.iloc[-1]) and pd.notna(previous_close.iloc[-1]) and previous_close.iloc[-1] else None
    ...
    result["bb_breakout_cross_flag"] = bool(
        len(close) >= 22 and pd.notna(upper.iloc[-1]) and pd.notna(upper.iloc[-2])
        and close.iloc[-2] <= upper.iloc[-2] and close.iloc[-1] > upper.iloc[-1]
    )
    result["bb_breakout_hold_flag"] = bool(
        len(close) >= 23 and all(pd.notna(upper.iloc[-i]) and close.iloc[-i] >= upper.iloc[-i] for i in (1, 2))
    )
    if len(bandwidth.dropna()) >= 100:
        ...
        previous = bandwidth.dropna().iloc[-2] if len(bandwidth.dropna()) >= 2 else np.nan
        result["bb_bandwidth_expansion_ratio"] = float(bandwidth.iloc[-1] / previous) if pd.notna(previous) and previous > 0 and pd.notna(bandwidth.iloc[-1]) else None
    else:
        result["bb_bandwidth_p15_100"] = None
        result["bb_squeeze_fraction_8"] = None
        result["bb_bandwidth_expansion_ratio"] = None
    ...
    result["stoch_bull_cross_flag"] = bool(
        len(stoch_k) >= 2 and pd.notna(stoch_k.iloc[-1]) and pd.notna(stoch_d.iloc[-1])
        and pd.notna(stoch_k.iloc[-2]) and pd.notna(stoch_d.iloc[-2])
        and stoch_k.iloc[-2] <= stoch_d.iloc[-2] and stoch_k.iloc[-1] > stoch_d.iloc[-1]
    )

    avg_volume = volume.rolling(20).mean()
    result["rvol20"] = float(volume.iloc[-1] / avg_volume.iloc[-1]) if len(close) >= 20 and avg_volume.iloc[-1] else None
    result["rvol20_3d_avg"] = float((volume / avg_volume.replace(0, np.nan)).tail(3).mean()) if len(close) >= 22 else None
    ...
    result["volatility_1y_pct"] = float(daily.rolling(252).std().iloc[-1] * np.sqrt(252) * 100) if len(close) >= 253 else None
    ...
    result["positive_reversal_flag"] = bool(
        result.get("rsi14") is not None and 30 <= result["rsi14"] < 70
        and result.get("macd_hist") is not None and result["macd_hist"] > 0
        and result.get("mm20") is not None and close.iloc[-1] > result["mm20"]
    )
