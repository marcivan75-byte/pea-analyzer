...
def _min_confidence(*values: object) -> str:
    grades = [_confidence_grade(v) for v in values]
    return min(grades, key=lambda item: CONFIDENCE_ORDER[item]) if grades else "D"
...
def _flow_price_score(flow_rate: float | None, price_return: float | None) -> tuple[float | None, str]:
    ...
    flow_eps = 0.001
    price_eps = 0.005
    if flow_rate > flow_eps and price_return > price_eps:
    ...
    if abs(flow_rate) <= flow_eps and price_return > price_eps:
...
def _derive_period_return(current: pd.Series, previous: pd.Series) -> float | None:
    ...
    current_price = _num(current.get("market_price"))
    previous_price = _num(previous.get("market_price"))
...
def compute_daily_flows(snapshot_history: pd.DataFrame) -> pd.DataFrame:
    ...
    for instrument_id, group in frame.groupby("instrument_id", sort=False):
        group = group.sort_values("as_of").reset_index(drop=True)
        for index, current in group.iterrows():
            row = current.to_dict()
            row["flow"] = np.nan
            row["flow_method"] = "FIRST_OBSERVATION"
            row["flow_confidence"] = _confidence_grade(current.get("confidence"))
            row["organic_flow_rate"] = np.nan
            row["period_return"] = np.nan
            if index > 0:
                previous = group.iloc[index - 1]
                flow, method, confidence = _daily_flow(current, previous)
                row["flow"] = flow if flow is not None else np.nan
                row["flow_method"] = method
                row["flow_confidence"] = confidence
                previous_aum = _num(previous.get("aum"))
...
def _current_instruments(rolling: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    current["peer_relative_raw"] = pd.to_numeric(current["organic_flow_rate_20d"], errors="coerce") - current.groupby(
        "economic_family"
    ...
    for _, row in current.iterrows():
        score, state = _flow_price_score(_num(row.get("organic_flow_rate_20d")), _num(row.get("price_return_20d")))
        confirmation_scores.append(score if score is not None else np.nan)
    ...
    score_columns = {
        ...
        "flow_20d": "score_flow_20d",
        "flow_60d": "score_flow_60d",
    ...
    current["efs_status"] = pd.cut(
        ...
        labels=["STRONG_OUTFLOW", "OUTFLOW", "NEUTRAL", "MODERATE_INFLOW", "STRONG_INFLOW", "EXCEPTIONAL_ACCUMULATION"],
        right=False,
    ).astype("string")
...
def add_pea_overlay(instruments: pd.DataFrame, families: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    global_family_score = families.groupby("economic_family")["family_flow_score"].mean().to_dict() if not families.empty else {}
    pea_family_score = (
        result[result["is_pea"].fillna(False).astype(bool)]
        .groupby("economic_family")["efs_shadow"]
        .mean()
        .to_dict()
    )
    ...
    for _, row in result.iterrows():
        ...
        overlay = _weighted_mean(
            [
                ...
                (_num(global_family_score.get(row.get("economic_family"))), weights["global_family"]),
                (_num(pea_family_score.get(row.get("economic_family"))), weights["pea_family"]),
            ]
        )
        overlays.append(round(overlay, 4) if overlay is not None else np.nan)
...
def build_rotation_scores(instruments: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    for label, group in eligible.groupby("sector_or_theme", sort=True):
        rates_5 = pd.to_numeric(group["organic_flow_rate_5d"], errors="coerce")
        rates_20 = pd.to_numeric(group["organic_flow_rate_20d"], errors="coerce")
        rates_60 = pd.to_numeric(group["organic_flow_rate_60d"], errors="coerce")
        acceleration = pd.to_numeric(group["flow_acceleration"], errors="coerce")
        persistence = pd.to_numeric(group["positive_days_20d_pct"], errors="coerce")
        ...
        rows.append(
            {
                ...
                "breadth_positive_20d_pct": rates_20.gt(0).mean() * 100.0 if rates_20.notna().any() else np.nan,
                "mean_acceleration": acceleration.mean(),
                "mean_persistence_20d_pct": persistence.mean(),
                "regional_confirmation_pct": (regions / total_regions * 100.0) if total_regions else np.nan,
                "mean_price_return_20d": price.mean(),
                "flow_price_confirmation_score": flow_price,
                "flow_price_state": state,
            }
        )
    rotations = pd.DataFrame(rows)
    rotations["score_aggregate_flow"] = (
        _rank_score(rotations["mean_rate_5d"]).mul(0.35)
        + _rank_score(rotations["mean_rate_20d"]).mul(0.40)
        + _rank_score(rotations["mean_rate_60d"]).mul(0.25)
    )
    ...
    for _, row in rotations.iterrows():
        ...
        srfs_scores.append(round(score, 4) if score is not None else np.nan)
    rotations["srfs_shadow"] = srfs_scores
...
def build_gold_crypto_summary(instruments: pd.DataFrame, cfg: dict) -> dict:
    ...
    gold_payload = {}
    if not gold.empty:
        us = group_score(gold, gold["region"].astype(str).str.upper().eq("US"))
        eu = group_score(gold, gold["region"].astype(str).str.upper().isin(["EU", "EUROPE"]))
        miners = group_score(gold, gold["asset_class"].astype(str).str.upper().eq("GOLD_MINERS_ETF"))
        price_score_value = pd.to_numeric(gold["score_flow_price_confirmation"], errors="coerce").mean()
        price_score = None if pd.isna(price_score_value) else float(price_score_value)
        weights = cfg["gold_flow_composite_weights"]
        composite = _weighted_mean(
            [(us, weights["us_physical"]), (eu, weights["eu_physical"]), (miners, weights["gold_miners"]), (price_score, weights["price_confirmation"])]
        )
        gold_payload = {
            "us_physical_score": us,
            "eu_physical_score": eu,
            "gold_miners_score": miners,
            "price_confirmation_score": price_score,
    ...
    if not crypto.empty:
        for family, group in crypto.groupby("economic_family"):
            score = pd.to_numeric(group["efs_shadow"], errors="coerce").mean()
            crypto_payload[str(family)] = {
                "instrument_count": int(group["instrument_id"].nunique()),
...
def build_flow_computation(snapshot_history: pd.DataFrame, cfg: dict) -> FlowComputation:
    ...
    summary = build_gold_crypto_summary(instruments, cfg)
    diagnostics = {
        "version": cfg["version"],
        "mode": cfg["mode"],
