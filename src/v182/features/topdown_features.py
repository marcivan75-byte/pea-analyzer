...
def _record_news_diagnostics(specs: list[dict], results: dict, diagnostics: list[dict]) -> None:
    for spec in specs:
        score,error=results.get(spec["query"],(None,"GDELT_RESULT_MISSING"))
        if score is None:
            diagnostics.append({
                "kind":spec["kind"],"key":spec["key"],"query":spec["query"],
                "articles":0,"positive_hits":0,"negative_hits":0,"score":None,"error":error,
            })
            continue
        diagnostics.append({
            "kind":spec["kind"],"key":spec["key"],"query":spec["query"],
...
def build_topdown(actions: pd.DataFrame, etfs: pd.DataFrame, *, fred_api_key: str | None, instrument_news_top_n: int = 80) -> TopDownResult:
    ...
    for asset_class,target in (("ACTION",action_scores),("ETF",etf_scores)):
        ctx=contexts[asset_class]; frame=ctx["frame"]
        ...
        countries=ctx["countries"]; sectors=ctx["sectors"]
        for idx,row in frame.iterrows():
            ...
            if country in ctx["country_macro"]: item["funnel_country_macro_score"]=ctx["country_macro"][country]
            if country in country_news: item["funnel_country_news_score"]=country_news[country]
            if sector in sector_news: item["funnel_sector_news_score"]=sector_news[sector]
            ...
            for key,value in global_scores.items(): item[key]=value
            target[isin]=item
