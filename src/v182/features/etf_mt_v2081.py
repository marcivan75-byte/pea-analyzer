...
MARKET_PROXY_ID = "PEA_ETF_EQUAL_WEIGHT_PROXY"
...
def _aligned_returns(close: pd.Series, proxy_close: pd.Series, sessions: int = 252) -> pd.DataFrame:
    joined = pd.concat(
        [close.rename("etf"), proxy_close.rename("market")],
        axis=1,
        join="inner",
    ).dropna()
...
def compute_raw_features(frame: pd.DataFrame, market_proxy_close: pd.Series) -> dict[str, float]:
    """Compute the 38 V20.8.1 PIT features from daily OHLCV data."""
    ...
    downside_vol126 = (
        float(np.sqrt((negative126.pow(2)).mean()) * np.sqrt(252))
        if len(negative126)
        else np.nan
    )
    ...
    sharpe126 = (
        float(r126.mean() / r126_std * np.sqrt(252))
        if pd.notna(r126_std) and r126_std > 0
        else np.nan
    )
    sortino126 = (
        float(r126.mean() * 252 / downside_vol126)
        if pd.notna(downside_vol126) and downside_vol126 > 0
        else np.nan
    )
    ...
    values = {
        ...
        "perf_2m": _perf(close, 42),
        "perf_3m": perf_3m,
        "momentum_accel": perf_3m - previous_3m,
        "positive_days_63": float((returns.tail(63) > 0).mean()),
        "perf_6m": _perf(close, 126),
        "perf_9m": _perf(close, 189),
        "perf_1y": perf1y,
        "perf_3y": _perf(close, 756),
        "relative_strength_6m": _perf(close, 126) - proxy_perf6,
        "relative_strength_1y": perf1y - proxy_perf1y,
        "dist_sma50": float(close.iloc[-1] / sma50.iloc[-1] - 1.0),
        "dist_sma100": float(close.iloc[-1] / sma100.iloc[-1] - 1.0),
        "dist_sma200": float(close.iloc[-1] / sma200.iloc[-1] - 1.0),
        "slope_sma50": float(sma50.iloc[-1] / sma50.iloc[-22] - 1.0),
        "slope_sma200": float(sma200.iloc[-1] / sma200.iloc[-64] - 1.0),
        "trend_alignment": float(trend_alignment),
        "efficiency_63": float(efficiency63),
        "rsi_quality": float(rsi14),
        "macd_norm": float(macd.iloc[-1] / close.iloc[-1]),
        "macd_hist_norm": float((macd.iloc[-1] - signal.iloc[-1]) / close.iloc[-1]),
        "vol20": ann_vol20,
        "vol60": ann_vol60,
        "vol_1y": ann_vol1y,
        "downside_vol60": downside_vol60,
        "maxdd_6m": maxdd6,
        "maxdd_1y": maxdd1,
        "current_dd_1y": current_dd1,
        "tail_loss_126": float(r126.quantile(0.05)) if len(r126) else np.nan,
        "sharpe_126": sharpe126,
        "sortino_126": sortino126,
        "calmar_252": calmar252,
        "gain_to_pain_126": gain_to_pain,
        "notional_volume20": notional_volume20,
        "rvol20": float(volume.iloc[-1] / avg_vol20) if avg_vol20 > 0 else np.nan,
        "volume_trend": float(avg_vol20 / avg_vol126 - 1.0) if avg_vol126 > 0 else np.nan,
        "beta252": beta252,
        "corr_market252": corr252,
    }
    return {
        name: float(value)
...
def build_equal_weight_market_proxy(histories: Mapping[str, pd.DataFrame]) -> pd.Series:
    """Build the versioned PEA-ETF equal-weight proxy from historical closes."""
    ...
    daily_returns = panel.pct_change(fill_method=None)
    proxy_returns = daily_returns.mean(axis=1, skipna=True).fillna(0.0)
...
def _criterion_scores(raw: pd.DataFrame, criteria_cfg: Mapping[str, dict]) -> pd.DataFrame:
    ...
    for name, spec in criteria_cfg.items():
        ...
        if direction == "HIGH":
            ...
            quality = -(values - optimum).abs()
            scores[name] = quality.rank(method="average", pct=True) * 100.0
        else:
...
def market_regime(raw: pd.DataFrame, market_proxy_close: pd.Series) -> tuple[MarketRegime, dict]:
    ...
    if valid.empty:
        ...
        return regime, {
            ...
            "median_perf_6m": None,
            "median_vol20": None,
            "market_trend200": None,
            "market_proxy": MARKET_PROXY_ID,
        }
    ...
    trend200 = (
        float(proxy.iloc[-1] / sma200.iloc[-1] - 1.0)
        if len(proxy) >= 200 and pd.notna(sma200.iloc[-1])
        else np.nan
    )
    regime = MarketRegime(
        breadth200=breadth200,
        median_perf_1m=median_perf1,
        median_perf_6m=median_perf6,
        market_above_sma200=bool(pd.notna(trend200) and trend200 > 0),
    )
    return regime, {
        "breadth50": breadth50,
        "breadth200": breadth200,
        "median_perf_1m": median_perf1,
        "median_perf_6m": median_perf6,
        "median_vol20": _safe_float(median_vol20),
        "market_trend200": _safe_float(trend200),
        "market_proxy": MARKET_PROXY_ID,
    }
...
def score_snapshot(
    ...
    criteria_cfg = mt_config["dynamic_criteria"]
    expected = list(criteria_cfg)
    if len(expected) != 38:
        raise ValueError(f"V20.8.1 requires exactly 38 dynamic criteria, got {len(expected)}")

    proxy = build_equal_weight_market_proxy(histories)
    if proxy.empty:
        raise ValueError("cannot build ETF market proxy from OHLCV histories")
    ...
    for instrument_id, frame in histories.items():
        ...
        stale_days = (
            int((pd.Timestamp(global_latest).normalize() - pd.Timestamp(latest).normalize()).days)
            if global_latest is not None
            else 999
        )
        row = {
            "instrument_id": instrument_id,
            "feature_as_of": str(pd.Timestamp(latest).date()),
            "history_sessions": int(len(frame)),
            "staleness_days": stale_days,
            **{name: features.get(name, np.nan) for name in expected},
        }
    ...
    if snapshot.empty:
        raise ValueError("no ETF histories available for V20.8.1 scoring")
    ...
    if not complete.empty:
        ...
        final_scores = (
            float(mt_config["score"]["score_raw_weight"]) * raw_scores
            + float(mt_config["score"]["cross_section_rank_weight"]) * rank_scores
        )
        ...
        for _, row in snapshot[snapshot["criteria_complete"]].iterrows():
            ...
            candidates.append(
                Candidate(
                    ...
                    score_rank_pct=float(row["score_rank_pct"]),
                    exposure_group=_exposure_group(reference_row),
                )
            )
    ...
    threshold = float(mt_config["score"]["selection_threshold"])
    ...
    if not regime_allowed:
        ...
        snapshot.loc[
            snapshot["criteria_complete"] & (snapshot["score_final"] >= threshold),
            "decision",
        ] = "WATCH_NOT_TOP2"
    ...
    metadata_cols = [
        c for c in (
            "isin", "name", "provider", "category", "official_benchmark",
            "geo_exposure", "ticker_yahoo_final", "yahoo_ticker",
            "ter_pct", "aum_m", "fund_total_assets_eur_m", "dividend_yield_pct",
            "morningstar_rating", "risk_indicator",
        )
        if c in etf_reference.columns
    ]
    ...
    snapshot = snapshot.sort_values(
        ["selected", "score_final", "instrument_id"],
        ascending=[False, False, True],
        na_position="last",
    ).reset_index(drop=True)

    summary = {
        "version": mt_config.get("version", "20.8.1"),
        "module": "ETF_MT_HIGH_PRECISION",
        "as_of": str(pd.Timestamp(global_latest).date()) if global_latest is not None else None,
        "market_proxy": MARKET_PROXY_ID,
        "universe_histories": len(histories),
        "scorable_etfs": int(snapshot["criteria_complete"].sum()),
        "blocked_data_etfs": int((~snapshot["criteria_complete"]).sum()),
        "regime": {**asdict(regime), **regime_metrics, "allowed": regime_allowed},
        "selected": [
            {
                "isin": candidate.instrument_id,
        ...
        "top_n": int(mt_config["score"]["top_n"]),
        "target_win_rate": float(mt_config["objective"]["target_win_rate"]),
        "target_win_rate_guaranteed": bool(mt_config["objective"]["guaranteed"]),
    }
...
def load_histories_from_cache(
    cache_dir: str | Path,
    ticker_to_isin: Mapping[str, str],
...
def write_outputs(snapshot: pd.DataFrame, summary: Mapping, output_dir: str | Path) -> dict[str, str]:
    ...
    csv_path = output / "V20.8.1_ETF_MT_RANKING.csv"
    json_path = output / "V20.8.1_ETF_MT_SUMMARY.json"
