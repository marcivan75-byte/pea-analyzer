...
VERSION = "TCT_V24.3.0_DAILY_WEEKLY_TRADER_TOOLS_SHADOW"
...
def _android_summary(frame: pd.DataFrame, generated_at: str) -> str:
    lines = [
        "# TCT V24.3 — Daily/Weekly Trader Tools SHADOW",
        "",
        f"Généré UTC : {generated_at}",
        "Horizon : quotidien à quelques séances / semaine. Pas de day trading.",
        "Données : OHLCV daily déjà collectées ; weekly dérivé du daily. Aucun 5 min, aucun quasi temps réel.",
    ...
    for _, row in work.iterrows():
        ...
        lines.append(
            f"- **{name}** — entrée {row.get('entry_state')} ({entry_txt}) — sortie {row.get('exit_state')} ({exit_txt})"
        )
...
def run(root: Path = ROOT) -> dict:
    cfg = json.loads((root / "config" / "TCT_V24_3_0_DAILY_TRADER_SHADOW.json").read_text(encoding="utf-8"))
    ...
    output_path = outdir / "TCT_DAILY_TRADER_V24_3_0_SHADOW.csv"
    ...
    (auditdir / "TCT_DAILY_TRADER_V24_3_0_AUDIT.json").write_text(
