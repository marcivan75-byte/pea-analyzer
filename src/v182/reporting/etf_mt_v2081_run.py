...
def _attach_selected_source_context(dynamic_snapshot, etf_with_tickers, root: Path):
    if "dynamic_selected" not in dynamic_snapshot:
        return dynamic_snapshot, {"status": "NO_DYNAMIC_SELECTION_COLUMN"}, source_gate_summary(dynamic_snapshot)
    selected = dynamic_snapshot[dynamic_snapshot["dynamic_selected"].fillna(False).astype(bool)].copy()
    if selected.empty:
        return dynamic_snapshot, {"status": "NO_PRESELECTED_ROWS"}, source_gate_summary(selected)
    selected["asset_class"] = "ETF"; selected["horizon"] = "MT"; selected["decision"] = "SHADOW_CANDIDATE"; selected = attach_master_identity(selected, None, etf_with_tickers)
    enriched, context = enrich_selected_rows(selected, root, profile="ETF_MT", network_policy="CACHE_ONLY", persist_outputs=False)
    enriched = apply_source_confirmation_gate(enriched, _read_contract(root)); source_columns = [c for c in enriched.columns if c.startswith("investing_") or c.startswith("boursorama_") or c.startswith("source_") or c == "ci_source_eligible"]
    if not source_columns: return dynamic_snapshot, context, source_gate_summary(enriched)
    return dynamic_snapshot.merge(enriched[["isin"] + source_columns].drop_duplicates("isin"), on="isin", how="left"), context, source_gate_summary(enriched)


def run(root: Path = ROOT, *, history_cache_dir: str | Path | None = None, refresh_history: bool = True, refresh_if_reuse_cache_missing: bool = False) -> dict:
    inputs = root/"inputs"; config = root/"config"; outputs = root/"outputs"/"etf_mt_v2081"; cache = Path(history_cache_dir) if history_cache_dir is not None else root/"data"/"cache"/"etf_mt_v2081"
    ...
    etf_df = load_master(inputs/"V18.2_PEA_ETF_MASTER.csv"); etf_with_tickers, gaps = resolve_etf_tickers(etf_df, config/"V18.2_ETF_TICKER_MAP.csv"); valid = etf_with_tickers.dropna(subset=["yahoo_ticker"]).copy(); valid = valid[valid["yahoo_ticker"].astype(str).str.strip().ne("")]; tickers = valid["yahoo_ticker"].astype(str).tolist()
    if not tickers: raise RuntimeError("ETF_MT_V20_8_NO_VALID_TICKERS")
    yf_cfg = base_cfg["yfinance"]; result = None; reuse_fallback = False; cache_has_history = any(cache.glob("history_*.parquet"))
    if refresh_history or (refresh_if_reuse_cache_missing and not cache_has_history):
        reuse_fallback = not refresh_history; result = download_history(tickers=tickers, cache_dir=str(cache), period=yf_cfg.get("history_period", "5y"), interval=yf_cfg.get("interval", "1d"), batch_size=int(yf_cfg.get("etf_batch_size", 50)), auto_adjust=bool(yf_cfg.get("auto_adjust", True)), start=yf_cfg.get("history_start", "2023-01-01"), rolling_months=int(yf_cfg.get("history_rolling_months", 60)))
    elif not cache_has_history: raise RuntimeError(f"ETF_MT_V20_8_REUSE_CACHE_EMPTY:{cache}")
    ...
    if not histories: raise RuntimeError(f"ETF_MT_V20_8_NO_USABLE_HISTORY:{cache}")
    ...
    if result is None:
        ...
        mode = "PRIMARY_CACHE_MISS_INCREMENTAL_REFRESH" if reuse_fallback else "ETF_MT_INCREMENTAL_CACHE_REFRESH"; download_summary = {"requested": result.requested, "successful": len(result.successful), "failed": len(result.failed), "ticker_mapping_gaps": int(len(gaps)), "mode": mode, "network_collection_executed": True, "network_requests_avoided": 0, "cache_dir": str(cache)}
    ...
    dynamic_snapshot, source_context, gate_summary = _attach_selected_source_context(dynamic_snapshot, etf_with_tickers, root); dynamic_summary["selected_source_context"] = source_context; dynamic_summary["source_confirmation_gate"] = gate_summary; dynamic_summary["source_network_policy"] = "CACHE_ONLY"; dynamic_summary["source_context_score_influence"] = 0.0; dynamic_summary["source_context_weights_unchanged"] = True
    ...
    print(f"ETF MT V20.8.2 — {dynamic_summary['scorable_etfs']} ETF scorables, selected={len(dynamic_summary['selected'])}, source_context=CACHE_ONLY")
