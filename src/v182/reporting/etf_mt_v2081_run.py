...
def run() -> dict:
    ...
    yf_cfg=base_cfg["yfinance"]; result=download_history(tickers=tickers,cache_dir=str(CACHE),period=yf_cfg.get("history_period","5y"),interval=yf_cfg.get("interval","1d"),batch_size=int(yf_cfg.get("etf_batch_size",50)),auto_adjust=bool(yf_cfg.get("auto_adjust",True)),start=yf_cfg.get("history_start","2023-01-01"),rolling_months=int(yf_cfg.get("history_rolling_months",60)))
    ticker_to_isin={str(ticker):str(isin) for ticker,isin in zip(valid["yahoo_ticker"],valid["isin"]) if str(ticker).strip() and str(isin).strip()}; histories=load_histories_from_cache(CACHE,ticker_to_isin); scoring_histories=sanitize_histories(histories)

    strict_snapshot,strict_summary=score_snapshot_integrity(histories,etf_with_tickers,mt_cfg); run_id=os.environ.get("V2081_RUN_ID") or datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"); strict_summary["run_id"]=run_id; strict_summary["download"]={"requested":result.requested,"successful":len(result.successful),"failed":len(result.failed),"ticker_mapping_gaps":int(len(gaps))}; strict_summary["status"]="ACTIVE_REFERENCE_SCORING_NO_REAL_ORDERS"; strict_paths=write_outputs(strict_snapshot,strict_summary,OUTPUTS); strict_summary["outputs"]=strict_paths
