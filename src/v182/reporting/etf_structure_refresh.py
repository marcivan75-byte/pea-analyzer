...
def run(root: Path=ROOT) -> dict:
    ...
    payload={
        "status":"SUCCESS","version":"V21.18_ETF_STRUCTURAL_BENCHMARK_STATE",
        "generated_at_utc":datetime.now(timezone.utc).isoformat(),"source":str(source.relative_to(root)),
        "tickers_requested":len(ticker_to_isin),"state_replay":state_replay_diag,
        "state_replay_merge_quarantined":len(state_replay_quarantined),
        "structural_source":structural_metrics,"structural_merge_observations":len(structural_observations),
        "structural_changed_cells":int(structural_changed),"structural_merge_quarantined":len(structural_quarantined),
        "inception_source":inception_metrics,"inception_merge_observations":len(inception_observations),
        "inception_changed_cells":int(inception_changed),"inception_merge_quarantined":len(inception_quarantined),
        "yfinance_raw_observations":len(raw_observations),"yfinance_merge_observations":len(observations),
        ...
        "governance":{
            ...
            "benchmark_price_symbol_inference":False,"tracking_error_activation":False,
            "stress_calibration_weight":0.0,"t1_t2_scope_changed":False,"holdout_opened":False,
        },
    }
