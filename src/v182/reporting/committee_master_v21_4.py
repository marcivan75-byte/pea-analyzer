...
def run(root:Path=ROOT)->dict:
    ...
    reference["key"]=_key(reference)
    refmap=reference.set_index("key",drop=False)

    action_mask=(decisions["asset_class"].astype(str)=="ACTION") & decisions["horizon"].astype(str).isin(HORIZONS)
    actions_current=decisions[action_mask].copy(); actions_current["key"]=_key(actions_current)
    rows=[]
    for idx,row in actions_current.iterrows():
        ...
        decisions.at[idx,"notes"]=(note+" | FINAL ACTION DECISION uses frozen V21.0 reference weights; V21.4 enriched score is challenger-only pending PIT/OOS validation.").strip(" |")
    ...
    challenger_view=decisions.copy()
    mask=(challenger_view["asset_class"].astype(str)=="ACTION") & challenger_view["horizon"].astype(str).isin(HORIZONS)
    ...
    summary["action_dual_track"]={
        "status":"ACTIVE_REFERENCE_PLUS_SHADOW_CHALLENGER",
        "reference_version":reference_reg.get("version"),
        "challenger_version":challenger_reg.get("version"),
        "final_decision_source":"REFERENCE",
        "comparison_rows":int(len(comparison)),"decision_divergences":divergences,
        "reference_buy_count":ref_buy,"challenger_buy_count":chal_buy,
        "performance_attribution":"NONE_TO_V21_4_CHALLENGER_UNTIL_DEDICATED_PIT_OOS_BACKTEST",
    }
    ...
    summary.setdefault("notes",[]).append("Actions use V21.0 frozen weights as final reference decision; V21.4 enriched score is exported as shadow challenger until PIT/OOS validation.")
