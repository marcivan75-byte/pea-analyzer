...
def _completed_daily_history(history: pd.DataFrame, cfg: dict, now: datetime | None = None) -> tuple[pd.DataFrame, bool]:
    ...
    if len(idx) == 0 or pd.isna(idx[-1]):
        return history, False
    if pd.Timestamp(idx[-1]).date() == current.date() and current.hour < guard_hour:
...
def _fingerprint(row: pd.Series | dict) -> str:
    getter = row.get
    canonical = "|".join(f"{field}={_canonical_value(getter(field))}" for field in FINGERPRINT_FIELDS)
...
def _label_outcomes(ledger: pd.DataFrame, histories_by_isin: dict[str, pd.DataFrame], horizons: list[int]) -> pd.DataFrame:
    ...
    for horizon in horizons:
        for field in (f"return_{horizon}d_pct",):
            if field not in out.columns:
                out[field] = np.nan
    for field in ("entry_date", "entry_open", "mfe_20d_pct", "mae_20d_pct"):
        if field not in out.columns:
            out[field] = np.nan if field != "entry_date" else pd.NA
    ...
    for idx, row in out.iterrows():
        ...
        try:
            ...
            continue
        future = history[pd.to_datetime(history.index).date > snapshot_date]
        if future.empty:
            continue
        ...
        if len(future) >= 20:
            ...
            max_high = _finite(pd.to_numeric(window["high"], errors="coerce").max())
            min_low = _finite(pd.to_numeric(window["low"], errors="coerce").min())
            if max_high is not None:
                out.at[idx, "mfe_20d_pct"] = (max_high / entry_open - 1.0) * 100.0
            if min_low is not None:
                out.at[idx, "mae_20d_pct"] = (min_low / entry_open - 1.0) * 100.0
    return out
...
def _temporal_exit_confirmation(output: pd.DataFrame, previous: pd.DataFrame) -> pd.DataFrame:
    ...
    if not previous.empty and {"isin", "exit_state"}.issubset(previous.columns):
        prev_map = dict(zip(previous["isin"].astype(str).str.upper(), previous["exit_state"].astype(str)))
    final_states: list[str] = []
    reasons: list[str] = []
    for _, row in out.iterrows():
        raw = str(row.get("exit_state_raw") or "")
        isin = str(row.get("isin") or "").upper()
        prior = str(prev_map.get(isin, ""))
        if raw == "EXIT_RISK_HIGH_CANDIDATE_SHADOW":
            if prior in {"EXIT_WATCH_SHADOW", "EXIT_RISK_HIGH_SHADOW"}:
                final_states.append("EXIT_RISK_HIGH_SHADOW")
                reasons.append("TEMPORAL_CONFIRMATION_AFTER_PRIOR_WARNING")
            else:
                final_states.append("EXIT_WATCH_SHADOW")
                reasons.append("AWAIT_TEMPORAL_CONFIRMATION")
        else:
...
def _validation_payload(ledger: pd.DataFrame, cfg: dict) -> dict:
    ...
    return_field = f"return_{primary}d_pct"
    if ledger.empty or return_field not in ledger.columns:
        return {"status": "IMMATURE_SHADOW", "reason": "NO_LABELED_PIT_ROWS", "promotion_authority": False}
    labeled = ledger[pd.to_numeric(ledger[return_field], errors="coerce").notna()].copy()
    labeled[return_field] = pd.to_numeric(labeled[return_field], errors="coerce")
    labeled["entry_score"] = pd.to_numeric(labeled.get("entry_score"), errors="coerce")
    entry_ready = labeled[labeled["entry_state"].astype(str).isin(["ENTRY_READY_SHADOW", "ENTRY_STRONG_SHADOW"])].copy()
    baseline_buy = labeled[labeled["baseline_ct_decision"].astype(str).eq("BUY_CANDIDATE")].copy()
    dates = labeled["snapshot_date"].astype(str).nunique() if "snapshot_date" in labeled.columns else 0
    isins = labeled["isin"].astype(str).nunique() if "isin" in labeled.columns else 0
    maturity = {
        "labeled_primary_rows": int(len(labeled)),
        "distinct_isins": int(isins),
        "distinct_snapshot_dates": int(dates),
        "entry_ready_primary_rows": int(len(entry_ready)),
    }
    mature = bool(
    ...
    def _win_rate(frame: pd.DataFrame) -> float | None:
        if frame.empty:
            return None
        return float((frame[return_field] > 0).mean() * 100.0)

    def _median(frame: pd.DataFrame, field: str) -> float | None:
        if frame.empty or field not in frame.columns:
            return None
        x = pd.to_numeric(frame[field], errors="coerce").dropna()
        return None if x.empty else float(x.median())

    challenger_win = _win_rate(entry_ready)
    baseline_win = _win_rate(baseline_buy)
    challenger_median = _median(entry_ready, return_field)
    baseline_median = _median(baseline_buy, return_field)
    false_positive = None if entry_ready.empty else float((entry_ready[return_field] <= 0).mean())
    correlation_sample = labeled.dropna(subset=["entry_score", return_field])
    spearman = None
    if len(correlation_sample) >= 3:
        spearman = _finite(correlation_sample["entry_score"].corr(correlation_sample[return_field], method="spearman"))
    challenger_mae = _median(entry_ready, "mae_20d_pct")
    baseline_mae = _median(baseline_buy, "mae_20d_pct")

    win_improvement = None if challenger_win is None or baseline_win is None else challenger_win - baseline_win
    median_improvement = None if challenger_median is None or baseline_median is None else challenger_median - baseline_median
    mae_degradation = None
    if challenger_mae is not None and baseline_mae is not None:
        mae_degradation = abs(min(challenger_mae, 0.0)) - abs(min(baseline_mae, 0.0))

    gates_cfg = vcfg["primary_metrics"]
    gates = {
        "win_rate_improvement": bool(win_improvement is not None and win_improvement >= float(gates_cfg["entry_ready_win_rate_improvement_points_vs_baseline"])),
        "median_return_improvement": bool(median_improvement is not None and median_improvement >= float(gates_cfg["median_return_20d_improvement_points_vs_baseline"])),
        ...
        "mae_guard": bool(mae_degradation is not None and mae_degradation <= float(vcfg["risk_guard"]["median_mae_20d_degradation_points_max"])),
    }
    ...
    return {
        ...
        "maturity": maturity,
        "mature": mature,
        "samples": {"challenger_entry_ready": int(len(entry_ready)), "baseline_buy": int(len(baseline_buy))},
        "metrics": {
            "challenger_win_rate_pct": challenger_win,
            "baseline_win_rate_pct": baseline_win,
            "win_rate_improvement_points": win_improvement,
            "challenger_median_return_20d_pct": challenger_median,
            "baseline_median_return_20d_pct": baseline_median,
            "median_return_improvement_points": median_improvement,
            "spearman_entry_score_return_20d": spearman,
            "false_positive_rate": false_positive,
            "challenger_median_mae_20d_pct": challenger_mae,
            "baseline_median_mae_20d_pct": baseline_mae,
            "mae_degradation_points": mae_degradation,
        },
        "gates": gates,
...
def _android_summary(frame: pd.DataFrame, validation: dict, generated_at: str) -> str:
    ...
    if frame.empty:
        lines.append("Aucun diagnostic CT exploitable.")
        return "\n".join(lines) + "\n"
    work = frame.copy()
    work["_entry"] = pd.to_numeric(work.get("entry_score"), errors="coerce")
    priority = work[work["entry_state"].astype(str).isin(["ENTRY_READY_SHADOW", "ENTRY_STRONG_SHADOW", "WAIT_RISK_SHADOW", "ENTRY_CONFLICT_SHADOW"])].copy()
    priority = priority.sort_values("_entry", ascending=False).head(15)
    if priority.empty:
        lines.append("Aucun ENTRY_READY/STRONG ou warning prioritaire sur ce run.")
        return "\n".join(lines) + "\n"
    for _, row in priority.iterrows():
        ...
        baseline = str(row.get("baseline_ct_decision") or "N/A")
        lines.append(f"- **{name}** — CT V22 {row.get('entry_state')} ({score_txt}) — baseline {baseline} — sortie {row.get('exit_state')}")
...
def run(root: Path = ROOT, now: datetime | None = None) -> dict:
    ...
    if actions.empty:
        ...
        errors.append("ACTION_MASTER_MISSING")
    else:
        ticker_col = "yahoo_ticker" if "yahoo_ticker" in actions.columns else "ticker"
        valid = actions[actions[ticker_col].notna()].copy() if ticker_col in actions.columns else pd.DataFrame()
        if valid.empty:
            output = pd.DataFrame()
            histories = {}
            mapping = {}
            errors.append("ACTION_TICKER_MAPPING_MISSING")
        else:
            valid[ticker_col] = valid[ticker_col].astype(str).str.strip()
            valid["isin"] = valid["isin"].astype(str).str.upper()
            valid = valid[~valid[ticker_col].isin({"", "nan", "None"})].drop_duplicates("isin")
            mapping = dict(zip(valid["isin"], valid[ticker_col]))
            ...
            baseline = baseline.drop_duplicates("isin").set_index("isin", drop=False)
            rows: list[dict] = []
            for _, base in valid.iterrows():
                isin = str(base["isin"]).upper()
                ticker = str(base[ticker_col])
                history = histories.get(isin)
                if history is None or history.empty:
                ...
                rows.append(
                    {
                        ...
                        "yahoo_ticker": ticker,
                        "baseline_ct_score": b.get("score"),
                        "baseline_ct_coverage_pct": b.get("coverage_pct"),
                        "baseline_ct_status": b.get("status"),
                        "baseline_ct_decision": b.get("decision"),
                        **snap,
    ...
    if not output.empty:
        ...
        _write_csv(output[["isin", "exit_state"]].drop_duplicates("isin", keep="last"), root / EXIT_STATE)
    ...
    ledger, added, fingerprint_mismatches = _append_first_snapshots(ledger, pit_candidates)
    ...
    validation.update(
        {
            ...
            "snapshot_fingerprint_mismatches": fingerprint_mismatches,
    ...
    android = mobile / "ANDROID_ACTION_CT_V22_SHADOW.md"
    android.write_text(_android_summary(output, validation, generated_at), encoding="utf-8")
    validation_mobile = mobile / "ANDROID_ACTION_CT_V22_PIT_VALIDATION.md"
    validation_mobile.write_text(
    ...
    payload = {
        "status": "FAIL_CLOSED_FINGERPRINT_MISMATCH" if fingerprint_mismatches else "SUCCESS_SHADOW_WITH_WARNINGS" if errors else "SUCCESS_SHADOW",
        "version": VERSION,
        "generated_at_utc": generated_at,
        "rows": int(len(output)),
