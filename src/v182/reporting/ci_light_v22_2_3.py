...
def _boursorama_consensus(row: pd.Series) -> str:
    # Actions currently expose analyst consensus. ETF Boursorama context does not.
    for field in (
...
def _evaluate(row: pd.Series) -> tuple[bool, list[str], str, str]:
    reasons: list[str] = []
    horizon = _norm(row.get("horizon"))
    ...
    if not boursorama:
        if asset == "ETF":
            reasons.append("ETF_BOURSORAMA_ANALYST_RECOMMENDATION_NOT_AVAILABLE")
        else:
    ...
    return not reasons, reasons, boursorama, investing
...
def _export_columns(frame: pd.DataFrame) -> list[str]:
    wanted = [
        ...
        "CI_LIGHT_BOURSORAMA_RECOMMENDATION", "CI_LIGHT_INVESTING_SIGNAL",
...
def _markdown(frame: pd.DataFrame, generated: str) -> str:
    lines = [
        ...
        "",
        "Filtre strict: candidat du process principal + recommandation Boursorama ACHETER/RENFORCER (ou équivalent BUY/STRONG_BUY) + Investing BUY/STRONG_BUY sur l'horizon correspondant.",
        "",
    ...
    for horizon in ("TCT", "CT", "MT"):
        ...
        subset = frame[frame["horizon"].map(_norm).eq(horizon)]
        if subset.empty:
            lines.extend(["Aucun instrument ne satisfait simultanément les deux filtres.", ""])
            continue
        for _, row in subset.iterrows():
            ...
            lines.append(
                ...
                f"confiance={confidence if confidence is not None else 'NA'} | "
...
def run(root: Path = ROOT) -> dict:
    ...
    for _, row in frame.iterrows():
        accepted, reasons, boursorama, investing = _evaluate(row)
        record = row.to_dict()
        ...
        record["CI_LIGHT_REASON"] = "PASS_BOURSORAMA_AND_INVESTING" if accepted else "|".join(reasons)
    ...
    payload = {
        ...
        "boursorama_positive_values": sorted(BOURSORAMA_POSITIVE),
        "investing_positive_values": sorted(INVESTING_POSITIVE),
        "investing_horizon_mapping": {"TCT": "DAILY", "CT": "WEEKLY", "MT": "MONTHLY"},
        "etf_without_explicit_boursorama_analyst_recommendation_excluded": True,
