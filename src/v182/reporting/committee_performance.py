...
def run(root: Path) -> dict:
    ...
    today=datetime.now(timezone.utc).date().isoformat(); buy_set=set(cfg["buy_decisions"]); allowed_assets={"ACTION","ETF"} if not cfg.get("gold_in_virtual_pea_book",False) else {"ACTION","ETF","GOLD"}
    ...
    for _,row in current.iterrows():
        decision=str(row.get("decision","") or ""); key=row["key"]
        ...
        sid=f"{today}|{key}"; new_signals.append({"signal_id":sid,"signal_date":today,"asset_class":row.get("asset_class"),"horizon":row.get("horizon"),"isin":isin,"name":row.get("name"),"sector":row.get("sector"),"entry_price":p,"entry_score":_num(row.get("score")),"entry_decision":decision})
    ...
    last_nav=float(pd.to_numeric(nav.get("nav_eur",pd.Series(dtype=float)),errors="coerce").dropna().iloc[-1]) if not nav.empty and pd.to_numeric(nav.get("nav_eur"),errors="coerce").notna().any() else float(cfg["initial_capital_eur"])
    ...
    fee=float(cfg["transaction_cost_per_side_pct"])/100.0; closed=[]
    for idx,pos in positions[positions.get("status",pd.Series(index=positions.index,dtype=str)).astype(str)=="OPEN"].iterrows():
        ...
        if stop_hit or decision_exit:
            proceeds=qty*p*(1.0-fee); cash+=proceeds; positions.at[idx,"status"]="CLOSED"; positions.at[idx,"close_date"]=today; positions.at[idx,"close_price"]=p; positions.at[idx,"exit_reason"]="STOP" if stop_hit else f"DECISION_{decision}"; closed.append(pos.get("position_id")); tx=pd.concat([tx,pd.DataFrame([{"date":today,"type":"SELL","position_id":pos.get("position_id"),"isin":isin,"price":p,"quantity":qty,"gross_eur":qty*p,"cost_eur":qty*p*fee,"reason":positions.at[idx,"exit_reason"]}])],ignore_index=True)

    open_positions=positions[positions["status"].astype(str)=="OPEN"].copy(); exposure=sum((_num(r.get("quantity")) or 0)* (prices.get(str(r.get("isin","") or ""),{}).get("price") or (_num(r.get("entry_price")) or 0)) for _,r in open_positions.iterrows()); equity=max(last_nav,cash+exposure)
    sector_exposure={}
    ...
    _style_xlsx(xlsx)
    return {"status":"SUCCESS","as_of":today,"xlsx":str(xlsx.relative_to(root)),"nav_eur":round(current_nav,2),"cumulative_performance_pct":round(cumulative,4),"open_positions":int(len(open_positions)),"signals":int(len(signals)),"live_orders_enabled":False}
