...
from v182.decision.committee_master import (
    criterion_coverage_report, decisions_from_scores, gold_adapter, load_registry,
...
def _write_effective_weights(outdir:Path, actions:pd.DataFrame, etfs:pd.DataFrame, actions_reg:dict, etf_reg:dict)->dict:
    parts=[]
    if not actions.empty: parts.append(effective_weight_report(actions,actions_reg,"ACTION",["CT","MT","LT","SHORT","TOP_DOWN"]))
    if not etfs.empty: parts.append(effective_weight_report(etfs,etf_reg,"ETF",["CT","LT","SHORT","TOP_DOWN"]))
...
def run(root:Path=ROOT)->dict:
    ...
    if not actions.empty:
        d,c,f=_safe_horizons(actions,actions_reg,"ACTION",["CT","MT","LT","SHORT","TOP_DOWN"]); parts.extend(d); coverage_parts.extend(c); horizon_failures.extend(f)
        try:
    ...
    if not etfs.empty:
        d,c,f=_safe_horizons(etfs,etf_reg,"ETF",["CT","LT","SHORT","TOP_DOWN"]); parts.extend(d); coverage_parts.extend(c); horizon_failures.extend(f)
    else: parts.append(_failed_horizon("ETF","CT/LT/SHORT/TOP_DOWN","V20.7.1/V20.7",RuntimeError("ETF master input missing")))
    ...
    try: parts.append(gold_adapter(root/gold_required))
        ...
        logger.exception("Gold adapter failed"); parts.append(_failed_horizon("GOLD","TACTICAL/STRATEGIC","GOLD_V1_1",exc)); horizon_failures.append({"asset_class":"GOLD","horizon":"ALL","error":type(exc).__name__,"detail":str(exc)[:240]})
    ...
    summary={"version":master_cfg["version"],"status":master_cfg["status"],"generated_at_utc":generated,"live_orders_enabled":False,"input_files":{"actions":str(actions_path.relative_to(root)) if actions_path else None,"etf":str(etf_path.relative_to(root)) if etf_path else None,"etf_mt":str(mt_path.relative_to(root)) if mt_path else None},"canonical_actions":canonical_audit,"registry_integrity":{"actions_universe_expected":1829,"actions_universe_loaded":int(len(actions)),"actions_criteria_expected":633,"actions_criteria_loaded":int(actions_reg.get("criteria_count",0)),"etf_fields_expected":268,"etf_fields_loaded":int(etf_reg.get("criteria_count",0)),"t1_t2_scope":"ACTION_TCT_ONLY","tct_formula_version":tct_cfg.get("formula_version"),"gold_reference_present":(root/gold_required).exists()},"dynamic_weighting":{"policy":"AVAILABLE_ACTIVE_CRITERIA_RENORMALIZED_TO_100","effective_weight_report":effective_weights,"minimum_coverage_gates_preserved":True,"neutral_imputation_forbidden":True},"tct_baseline":tct_baseline_audit,"tct_exact_timing":tct_exact_audit,"etf_structural_overlay":overlay_summary,"action_52w_rotation_overlay":action_summary,"status_counts":status_counts.to_dict("records"),"decision_counts":decision_counts.to_dict("records"),"missing_active_criteria_by_horizon":missing_by_horizon,"tct_shadow_status":tct_status,"horizon_failures":horizon_failures,"outputs":{"decisions":"outputs/committee_master/COMMITTEE_DECISIONS.csv","sector_ranking":"outputs/committee_master/SECTOR_RANKING.csv","criteria_coverage":"outputs/committee_master/CRITERIA_COVERAGE.csv","effective_weights_csv":"outputs/committee_master/EFFECTIVE_WEIGHTS_100.csv","effective_weights_xlsx":"outputs/committee_master/EFFECTIVE_WEIGHTS_100.xlsx","tct_baseline":"outputs/committee_master/TCT_BASELINE_V24_1_8.csv","tct_shadow":"outputs/committee_master/TCT_SHADOW_V24_1_7.csv","tct_state":"state/TCT_V24_1_7_T1_STATE.json"},"notes":["Committee enforces the exact V21.3 1829-Action whitelist recovered as 1429 canonical + 400 validated quarantine.","Available active criteria are renormalized to 100% per instrument while minimum coverage gates remain enforced.","TCT V24.1.8 dynamically renormalizes available active pillars; setup remains intentionally excluded and T1/T2 remain timing-only with zero baseline-score influence.","V24.1.8 and V21.3 Actions reweighting have no historical performance attribution until dedicated PIT backtests.","Action CT/MT/LT scores receive an explicit 52-week-high bonus/malus; far-from-high bonus requires recovery evidence upstream.","ETF Morningstar/risk remains a Committee layer; ETF MT historical 90.91% remains attributable only to the exact 38 PIT core unless a new dynamic challenger is separately validated.","Gold runtime replacement is handled by the Gold V1.1 Committee bridge."]}
