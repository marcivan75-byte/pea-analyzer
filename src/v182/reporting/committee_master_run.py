...
from v182.decision.committee_master import load_registry, decisions_from_scores, overlay_etf_mt, tct_adapter, gold_adapter, sector_ranking
...
def run(root: Path=ROOT) -> dict:
    ...
    if not actions.empty:
        parts.append(decisions_from_scores(actions, actions_reg, "ACTION", ["CT","MT","LT","SHORT","TOP_DOWN"]))
    else:
    ...
    if not etfs.empty:
        parts.append(decisions_from_scores(etfs, etf_reg, "ETF", ["CT","LT","SHORT","TOP_DOWN"]))
    else:
