...
def _public_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Return report-facing columns only; private merge metadata stays in CSV masters."""
    return frame[[c for c in frame.columns if not str(c).startswith("_")]].copy()
...
def _coverage(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in frame.columns:
        ...
        rows.append({
            "column": col,
            "observed": observed,
            "missing": len(frame) - observed,
            "coverage_pct": round(observed / len(frame) * 100, 1) if len(frame) else 0,
        })
...
def export_master_excel(frame: pd.DataFrame, path: str | Path, title: str) -> Path:
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        pd.DataFrame({
            "indicator": ["title", "rows", "columns"],
            "value": [title, len(public), len(public.columns)],
        }).to_excel(writer, sheet_name="Dashboard", index=False)
        public.to_excel(writer, sheet_name="Referentiel", index=False)
        _coverage(public).to_excel(writer, sheet_name="Couverture_colonnes", index=False)
    _format(path)
    return path
...
def export_run_report(before: dict, after: dict, quality_checks: list[dict], path: str | Path) -> Path:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    summary = []
    for universe in ("ACTION", "ETF"):
        summary.append({
            "universe": universe,
            "coverage_before_pct": before[universe]["coverage_pct"],
            "coverage_after_pct": after[universe]["coverage_pct"],
            "gain_points": round(after[universe]["coverage_pct"] - before[universe]["coverage_pct"], 2),
        })
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
