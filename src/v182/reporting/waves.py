...
def wave_history(df: pd.DataFrame, universe: str, cache_dir: str, cfg: dict) -> DownloadResult:
    valid = df[df["yahoo_ticker"].apply(lambda value: not is_missing(value))]
...
def resolve_etf_tickers(etf_df: pd.DataFrame, mapping_path: str | Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if mapping_file.exists():
        ...
        mapping = mapping[[column for column in ["isin", "yahoo_ticker"] if column in mapping.columns]].drop_duplicates("isin")
    else:
    ...
    merged["yahoo_ticker"] = merged["yahoo_ticker"].where(
        ~merged["yahoo_ticker"].apply(is_missing), native.values
    )
...
def _history_source(path: Path) -> tuple[str, int]:
    ...
    if "alphavantage" in name:
        return "ALPHA_VANTAGE", 0
...
def _history_rank(frame: pd.DataFrame, source_priority: int) -> tuple[int, int, int]:
    """Prefer the longest series, then freshness, then source priority."""
...
def wave3_derived_features(cache_dir: str, ticker_isin_map: dict[str, str], universe: str) -> list[dict]:
    ...
    indicators_by_isin: dict[str, tuple[dict, str]] = {}
    for isin, (_, frame, source_name) in best_frames.items():
        indicators = calculate_features(frame)
        if not indicators:
            continue
        indicators_by_isin[isin] = (indicators, source_name)
    ...
    for isin, (indicators, source_name) in indicators_by_isin.items():
        ...
        if indicators.get("perf_1y_pct") is not None:
            observations.append(
                _obs(
                    ...
                    isin,
                    "relative_strength",
                    round(indicators["perf_1y_pct"] - median_perf, 3),
                    source,
                    "C",
                )
            )
...
def _priority_actions(actions_df: pd.DataFrame, top_n: int = 300) -> pd.DataFrame:
    if "comite_status" in actions_df.columns:
        priority_df = actions_df[actions_df["comite_status"].isin(["COMMITTEE", "WATCH"])]
    else:
...
def fundamentals_availability(actions_df: pd.DataFrame, top_n: int = 300) -> tuple[int, int, float]:
    ...
    fields = [
        "per_ttm", "per_forward", "per_ttm_yf", "per_forward_yf", "pb", "roe_api", "roa",
...
def consensus_availability(actions_df: pd.DataFrame, top_n: int = 300) -> tuple[int, int, float]:
    priority_df = _priority_actions(actions_df, top_n)
    fields = [
        "consensus_rating", "consensus_score", "n_analysts", "target_price",
...
def wave4_info_actions(actions_df: pd.DataFrame, cfg: dict, top_n: int = 300) -> tuple[list[dict], list[dict], dict]:
    ...
    tickers = [ticker for ticker in ticker_to_isin if not is_missing(ticker)]
    ...
    meta = {
        ...
        "available_before_pct": pct_before,
        "attempted": len(tickers),
        "skipped_fresh": max(0, len(priority_df) - len(needs_refresh)),
        "refresh_days": refresh_days,
        "refreshed_tickers": len(refreshed),
...
def _rating_from_yf(row: pd.Series) -> tuple[str | None, float | None]:
    ...
    key_map = {
        "strong_buy": ("STRONG_BUY", 5.0),
        "buy": ("BUY", 4.0),
        "hold": ("HOLD", 3.0),
        "underperform": ("SELL", 2.0),
        "sell": ("SELL", 2.0),
        "strong_sell": ("STRONG_SELL", 1.0),
    }
    if key in key_map:
        return key_map[key]
    ...
    try:
        ...
        rating = (
            "STRONG_BUY" if score >= 4.5 else
            "BUY" if score >= 3.5 else
            "HOLD" if score >= 2.5 else
            "SELL" if score >= 1.5 else
            "STRONG_SELL"
        )
        return rating, score
...
def wave5_consensus_finnhub(
    ...
    for _, row in priority_df.iterrows():
        ...
        canonical = {
            ...
            "consensus_status": "OK_EXISTING_YF",
    ...
    securities = unresolved[[column for column in ["isin", "name", "yahoo_ticker"] if column in unresolved.columns]].to_dict("records")
    ...
    ticker_to_isin = {
        ticker: isin
        for ticker, isin in zip(unresolved["yahoo_ticker"], unresolved["isin"])
        if not is_missing(ticker)
    }
    finnhub_tickers: set[str] = set()
    for row in obs_raw:
    ...
    meta = {
        ...
        "available_before": available_before,
        "available_before_pct": pct_before,
        "normalized_yf_tickers": len(covered_isins),
        "attempted_finnhub": len(securities),
        "finnhub_success_tickers": len({ticker for ticker in finnhub_tickers if ticker}),
        "failures": len(failures),
    }
    return result, failures, meta
...
def wave6_etf_info(etf_with_tickers: pd.DataFrame, cfg: dict) -> tuple[list[dict], list[dict]]:
    valid = etf_with_tickers[etf_with_tickers["yahoo_ticker"].apply(lambda value: not is_missing(value))]
    ticker_to_isin = dict(zip(valid["yahoo_ticker"], valid["isin"]))
    obs_raw, failures = collect_info(
        list(ticker_to_isin),
    ...
    for _, row in rows.iterrows():
        ...
        try:
            response = requests.get(
                url_template.format(symbol=symbol, isin=isin),
                headers=headers,
                timeout=15,
            )
...
def wave7_official_validation(quarantine: list[dict], overrides_path: str | Path) -> list[dict]:
    ...
    for _, override in overrides.iterrows():
        match = [
            ...
            if item["isin"] == override["isin"] and item["field"] == override["field"]
        ]
        if match:
            resolved.append(
                _obs(
