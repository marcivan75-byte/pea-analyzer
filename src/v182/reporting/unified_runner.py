...
def _boursorama_attributed_import(root:Path)->dict:
    ...
    # The legacy/general loader remains responsible for Actions and attributed
    # CSV/XLSX. Its HTML ETF observations are deliberately discarded here and
    # regenerated by the stricter ETF loader below so a rendered '/7' can never
    # be misread as a risk score of 7/7.
    ...
    observations.extend(current_obs); observations.extend(bulk_obs); observations.extend(etf_obs)
    failures.extend(current_fail); failures.extend(bulk_fail); failures.extend(etf_fail)
    ...
    global_audit=write_collection_audit(
        ...
        source_context="Boursorama attributed HTML/CSV/XLSX intake after online refresh; FactSet consensus/forecasts + Morningstar ETF context; graphical ETF risk remains missing unless numerator is explicit",
    )
...
def run(root:Path=ROOT)->dict:
    ...
    payload={
        ...
        "governance":[
            ...
            "Attributed Boursorama ETF pages feed existing Morningstar rating/category fields; risk_indicator is retained only when the 1..7 numerator is explicitly observable, never inferred from a rendered '/7'.",
