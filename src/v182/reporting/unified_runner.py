...
def run(root:Path=ROOT)->dict:
    ...
    decision_tracks={"actions_final":"V21.0 frozen-weight reference on current 1829 universe + V21.6.3 CDC/post-selection context","actions_challenger":"V21.4 enriched shadow challenger","etf_mt_reference":"V20.8.1 exact 38-PIT core; Boursorama raw category rank shadow only","etf_mt_challenger":"V20.8.2 missing-data dynamic shadow","tct":"V24.1.8 baseline + exact V24.1.7 T1/T2 shadow","gold":"V1.1 shadow"}
    payload={
        ...
        "governance":[
            ...
            "Boursorama/Morningstar ETF annual category ranks are preserved as raw published ranks; no percentile is fabricated; the layer is shadow-only and does not modify the exact V20.8.1 MT score.",
            "Boursorama and Investing Action sheets are fetched only after preselection; Investing timeframe order is parsed explicitly when available and ambiguous ADR/venue matches are rejected; all postselection signals have zero decision influence until PIT/OOS validation.",
