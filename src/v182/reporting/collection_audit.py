...
def _field_status(frame: pd.DataFrame, asset_class: str, wave_id: str) -> pd.DataFrame:
    rows=[]; n=max(len(frame),1)
    identity={"isin","name"}
    for field in frame.columns:
        if field in identity:
            continue
        available=int((~frame[field].apply(is_missing)).sum())
        coverage=available/n*100.0
        ...
        rows.append({
            ...
            "coverage_pct":round(coverage,2),"source_concernee":source_hint(field),
...
def write_collection_audit(
    ...
    *,
    ...
    """Write an Excel inventory immediately after one collection wave.

    MISSING means 0 observed rows, PARTIAL means between 0 and 100%, AVAILABLE
    means complete coverage. Missing values are never neutral-imputed.
    """
    root=Path(output_root); root.mkdir(parents=True,exist_ok=True)
    inventory=pd.concat([
        _field_status(actions,"ACTION",wave_id),
        _field_status(etfs,"ETF",wave_id),
    ],ignore_index=True)
    missing=inventory[inventory["status"]=="MISSING"].copy()
    partial=inventory[inventory["status"]=="PARTIAL"].copy()
    available=inventory[inventory["status"]=="AVAILABLE"].copy()
    summary=pd.DataFrame([
        {"collection":wave_id,"asset_class":"ACTION","universe_rows":len(actions),"missing_fields":int((inventory.query("asset_class=='ACTION' and status=='MISSING'")).shape[0]),"partial_fields":int((inventory.query("asset_class=='ACTION' and status=='PARTIAL'")).shape[0]),"source_context":source_context},
        {"collection":wave_id,"asset_class":"ETF","universe_rows":len(etfs),"missing_fields":int((inventory.query("asset_class=='ETF' and status=='MISSING'")).shape[0]),"partial_fields":int((inventory.query("asset_class=='ETF' and status=='PARTIAL'")).shape[0]),"source_context":source_context},
    ])
    failures_df=pd.DataFrame(failures or [])
    safe="".join(ch if ch.isalnum() or ch in "_-" else "_" for ch in wave_id)[:40]
    ...
    with pd.ExcelWriter(path,engine="openpyxl") as writer:
        ...
        inventory[["field","asset_class","source_concernee"]].drop_duplicates().to_excel(writer,sheet_name="Sources",index=False)
        failures_df.to_excel(writer,sheet_name="Echecs_collecte",index=False)
    _format_excel(path)
    latest=root/"COLLECTION_DATA_AVAILABILITY_LATEST.xlsx"
    shutil.copyfile(path,latest)
    history_path=root/"COLLECTION_AUDIT_HISTORY.csv"
    hist=summary.copy(); hist["generated_at_utc"]=datetime.now(timezone.utc).isoformat()
    hist.to_csv(history_path,sep=";",encoding="utf-8-sig",index=False,mode="a",header=not history_path.exists())
    return str(path)
