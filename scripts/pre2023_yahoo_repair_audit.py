...
def _geometry(frame: pd.DataFrame) -> pd.Series:
    ...
    return observed & ((h < l) | (h < o) | (h < c) | (l > o) | (l > c) |
                       (o <= 0) | (h <= 0) | (l <= 0) | (c <= 0))


def _reject_degraded_repair(rawf: pd.DataFrame, repf: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    """Make provider repair monotone: never replace a valid raw OHLC bar by an invalid repaired bar."""
    if rawf.empty or repf.empty:
        return repf, 0

    raw_bad = _geometry(rawf)
    rep_bad = _geometry(repf)
    if raw_bad.empty or rep_bad.empty:
        return repf, 0

    raw_bad_dates = set(raw_bad.index[raw_bad])
    rep_bad_dates = set(rep_bad.index[rep_bad])
    degraded_dates = sorted((rep_bad_dates - raw_bad_dates) & set(rawf.index) & set(repf.index))
    if not degraded_dates:
        return repf, 0

    safe = repf.copy()
    raw_cols = {str(c).strip().lower(): c for c in rawf.columns}
    rep_cols = {str(c).strip().lower(): c for c in safe.columns}
    for key in ("open", "high", "low", "close"):
        if key in raw_cols and key in rep_cols:
            safe.loc[degraded_dates, rep_cols[key]] = rawf.loc[degraded_dates, raw_cols[key]].to_numpy()

    return safe, len(degraded_dates)
...
def main() -> None:
    ...
    for ticker in tickers:
        ...
        if (not rawf.empty) and repf.empty:
            rows.append({
                "ticker": ticker,
                "status": "REPAIR_ERROR",
                "detail": "raw history present but repaired history empty",
            })
            continue

        repf, rejected_degraded_rows = _reject_degraded_repair(rawf, repf)
        raw_bad = _geometry(rawf)
        rep_bad = _geometry(repf)
        ...
        rows.append({
            ...
            "raw_rows": int(len(rawf)),
            "repaired_rows": int(len(repf)),
            "raw_invalid": raw_count,
            "repaired_invalid": rep_count,
            ...
            "rejected_degraded_repair_rows": int(rejected_degraded_rows),
    ...
    summary = {
        ...
        "rejected_degraded_repair_rows": int(pd.to_numeric(ok.rejected_degraded_repair_rows, errors="coerce").fillna(0).sum()) if len(ok) else 0,
        "provider_repaired_flag_rows": int(pd.to_numeric(ok.provider_repaired_flag_rows, errors="coerce").fillna(0).sum()) if len(ok) else 0,
    }
    (out / "PRE2023_YAHOO_REPAIR_AUDIT.json").write_text(
