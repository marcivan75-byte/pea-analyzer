...
def test_v243_runner_requires_no_new_market_download():
    source = (ROOT / "src" / "v182" / "reporting" / "tct_daily_trader_shadow_run.py").read_text(encoding="utf-8")
...
def test_daily_workflow_replaces_v242_with_v243_and_purges_old_cache():
    workflow = (ROOT / ".github" / "workflows" / "committee_tct_ct_daily.yml").read_text(encoding="utf-8")
    assert "TCT V24.3 daily weekly trader-tools shadow" in workflow
    assert "python -m v182.reporting.tct_daily_trader_shadow_run" in workflow
    assert "ANDROID_TCT_DAILY_TRADER_SHADOW.md" in workflow
    assert "TCT_DAILY_TRADER_V24_3_0_AUDIT.json" in workflow
...
def test_v243_does_not_modify_canonical_tct_ct_runner():
