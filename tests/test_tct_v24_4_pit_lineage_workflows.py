...
def test_daily_workflow_persists_close_ledger_from_existing_cache_only():
    ...
    assert "Persist compact daily closes for V24.4 PIT" in workflow
    assert "python -m v182.reporting.tct_pit_close_ledger" in workflow
    assert "state/tct_context/TCT_DAILY_CLOSE_LEDGER.csv" in workflow
    step = workflow.split("- name: Persist compact daily closes for V24.4 PIT", 1)[1].split("- name:", 1)[0]
...
def test_catalyst_workflow_applies_lineage_before_validator():
    ...
    lineage_pos = workflow.index("Apply fail-closed V24.4 PIT lineage")
    validator_pos = workflow.index("Validate accumulated V24.4 PIT ledger")
    assert lineage_pos < validator_pos
    assert "python -m v182.reporting.tct_v24_4_pit_lineage" in workflow
    assert "TCT_V24_4_0_PIT_LINEAGE_AUDIT.json" in workflow


def test_lineage_config_forbids_synthetic_historical_replay_and_requires_quality():
    cfg = json.loads((ROOT / "config" / "TCT_V24_4_0_CATALYST_CONTEXT_SHADOW.json").read_text(encoding="utf-8"))
