...
def test_local_currency_market_cap_is_never_merged_as_eur(tmp_path: Path):
    ...
    assert stats["dropped_local_currency_observations"] == 2
...
def test_eur_market_cap_is_retained(tmp_path: Path):
    ...
    assert stats["retained_eur_observations"] == 2


def test_unknown_currency_is_not_assumed_eur(tmp_path: Path):
    ...
    assert "boursorama_market_cap_currency" not in fields
    assert stats["dropped_unknown_currency_observations"] == 1
