...
class FakePreopen:
    def enrich(self, df, as_of_date):
        out=df.copy(); out['gap_overnight']=0.01; out['preopen_data_status']='OK'; out['score_preopen']=out['EV_net']
        return out
...
def test_default_preopen_has_no_fake_market_data():
    candidates=pd.DataFrame({'ticker':['ABC'],'hebdo_score':[1.0]})
    out=PreopenEnricher().enrich(candidates,'2026-01-05T08:00:00+01:00')
    assert out.iloc[0]['preopen_data_status']=='BLOCK_DATA_PREOPEN_SOURCE_REQUIRED'
    assert pd.isna(out.iloc[0]['gap_overnight'])


def test_preopen_provider_validates_quote_time_and_true_gap():
    def provider(tickers, as_of):
        return pd.DataFrame([{'ticker':'ABC','prev_close':100,'preopen_price':103,'quote_time':'2026-01-05T07:55:00+01:00'}])
    out=PreopenEnricher(provider).enrich(pd.DataFrame({'ticker':['ABC'],'hebdo_score':[1.0]}),'2026-01-05T08:00:00+01:00')
    assert out.iloc[0]['preopen_data_status']=='OK'
    assert abs(out.iloc[0]['gap_overnight']-0.03)<1e-12
    assert abs(out.iloc[0]['preopen_boost']-0.15)<1e-12


def test_meta_pipeline_fail_closed_missing_feature():
    try:
        HebdoATMeta().run(make_features().drop(columns=['atr_14_pct'])); assert False
...
def test_meta_pipeline_fail_closed_missing_liquidity():
    try:
        HebdoATMeta().run(make_features().drop(columns=['adv_20m_eur'])); assert False
    except ValueError as e:
        assert 'BLOCK_DATA_META' in str(e)


def test_invalid_critical_rows_are_dropped_not_scored():
    df=make_features(20); df.loc[0,'close']=np.nan
...
def test_infinite_critical_values_are_dropped():
    df=make_features(20); df.loc[0,'vol_z']=np.inf
    out=HebdoATMeta().run(df)
...
def test_past_earnings_are_not_future_event_exclusions():
    ...
    row=pd.Series({'close':100,'sma200':90,'drawdown_4w':-0.02,'atr_14_pct':0.03,'vol_z':1,'days_to_earnings':-2,'adv_20m_eur':2_000_000,'mom_26w_sector':0})
...
def test_negative_ev_never_promoted():
    ...
    out=r.rank_batch(df)
    assert (out['EV_net']<0).all() and (out['tier']=='EXCLU').all()
...
def test_confirmation_blocks_ambiguous_duplicate_bars_without_dates():
    ...
    bars=pd.DataFrame([{'ticker':'ABC','open':100,'close':101,'vol_z':1},{'ticker':'ABC','open':101,'close':102,'vol_z':1}])
    out=ConfirmationEntry().filter_batch_j1(friday,bars)
    assert out.iloc[0]['enter_confirmed'] is None and out.iloc[0]['confirm_reason']=='BLOCK_DATA_NEXT_BAR_AMBIGUOUS'
...
def test_gap_through_stop_uses_open_loss():
    hit=FPEarlyExit().check_exit(100,{'open':85,'close':86,'low':84},1)
    assert hit[0] is True and abs(hit[2]-(-0.15))<1e-12


def test_meta_training_sparse_classes_blocks_safely():
    df=make_features(40); df['meta_label']=1
    result=MetaLabeler(label_horizon_days=2).train(df)
...
def test_meta_training_requires_temporal_evidence():
    ...
    try:
        MetaLabeler(label_horizon_days=2).train(df); assert False
    except ValueError as e:
...
def test_meta_training_uses_calendar_purged_oos_split():
    ...
    df['vol_z']=df['meta_label']*3.0
    result=MetaLabeler(label_horizon_days=10).train(df)
    assert result['status']=='TRAINED_PURGED_TEMPORAL_OOS'
    assert result['split_scheme']=='purged_calendar_time_train_cal_test'
    assert result['embargo_days']==10
    assert result['n_train']<n and result['n_test']>0


def test_lasso_requires_explicit_purge_gap():
    X=pd.DataFrame({'f1':np.arange(40,dtype=float)}); y=pd.Series(np.arange(40,dtype=float))
    try:
        lasso_select_features(X,y,cv=3); assert False
    except ValueError as e:
        assert 'purge_gap required' in str(e)


def test_lasso_uses_fold_local_scaling_purge_and_drops_missing_labels():
    n=100
    X=pd.DataFrame({'f1':np.linspace(-1,1,n),'f2':np.sin(np.linspace(0,8,n))})
    y=pd.Series(0.5*X['f1']+0.1*X['f2']); y.iloc[-3:]=np.nan
    result=lasso_select_features(X,y,cv=4,purge_gap=5)
    assert result['cv_scheme']=='TimeSeriesSplit_Pipeline_GridSearchCV_Purged'
    assert result['purge_gap']==5 and result['n_labeled']==97 and result['n_dropped_missing_label']==3
