...
def test_staged_orchestrator_preopen_confirmation_and_exit():
    ...
    assert len(confirmed)==1 and bool(confirmed.iloc[0]['enter_confirmed']) is True and confirmed.iloc[0]['process_stage']=='J1_CONFIRMATION'
...
def test_default_preopen_has_no_fake_market_data():
    out=PreopenEnricher().enrich(pd.DataFrame({'ticker':['ABC'],'hebdo_score':[1.0]}),'2026-01-05T08:00:00+01:00')
    assert out.iloc[0]['preopen_data_status']=='BLOCK_DATA_PREOPEN_SOURCE_REQUIRED' and pd.isna(out.iloc[0]['gap_overnight'])
...
def test_preopen_provider_validates_quote_time_and_true_gap():
    ...
    assert out.iloc[0]['preopen_data_status']=='OK' and abs(out.iloc[0]['gap_overnight']-0.03)<1e-12 and abs(out.iloc[0]['preopen_boost']-0.15)<1e-12
...
def test_confirmation_requires_dates_even_for_single_bar():
    ...
    assert out.iloc[0]['enter_confirmed'] is None and out.iloc[0]['confirm_reason']=='BLOCK_DATA_NEXT_BAR_DATE_REQUIRED'
...
def test_confirmation_blocks_stale_future_bar():
    ...
    out=ConfirmationEntry().filter_batch_j1(friday,bars); assert out.iloc[0]['enter_confirmed'] is None and 'STALE' in out.iloc[0]['confirm_reason']
...
def test_trailing_requires_prior_peak_not_current_bar_lookahead():
    ex=FPEarlyExit()
    no_prior=ex.check_exit(100,{'open':100.5,'close':104,'low':100.5},5)
    assert no_prior[0] is False
    active=ex.check_exit(100,{'open':100.5,'close':100.8,'low':100.5,'peak_pnl_prior':0.04},5)
    assert active[0] is True and active[1].startswith('TRAIL_BE_PRIOR_PEAK')
...
def test_meta_training_sparse_classes_blocks_safely():
    df=make_features(40); df['meta_label']=1; result=MetaLabeler(label_horizon_days=2).train(df)
...
def test_lasso_requires_explicit_purge_gap():
    ...
    except ValueError as e: assert 'purge_gap required' in str(e)


def test_lasso_uses_fold_local_scaling_purge_and_drops_missing_labels():
    n=100; X=pd.DataFrame({'f1':np.linspace(-1,1,n),'f2':np.sin(np.linspace(0,8,n))}); y=pd.Series(0.5*X['f1']+0.1*X['f2']); y.iloc[-3:]=np.nan
    result=lasso_select_features(X,y,cv=4,purge_gap=5)
    assert result['cv_scheme']=='TimeSeriesSplit_Pipeline_GridSearchCV_Purged' and result['purge_gap']==5
    assert result['n_labeled']==97 and result['n_dropped_missing_label']==3
