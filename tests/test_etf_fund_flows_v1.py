        ...
        for index, date in enumerate(dates):
            ...
            rows.append(
                _base_row(
                    instrument_id,
                    date.date().isoformat(),
                    universe="PEA_ETF" if is_pea else "EXTERNAL",
                    economic_family=family,
                    region=region,
                    sector_or_theme=theme,
...
def test_pea_universe_uses_isin_and_economic_benchmark_for_synthetic_etf():
    master = pd.DataFrame(
        [
            {
                "isin": "FR0014000001",
                "name": "Synthetic World PEA",
                "yahoo_ticker": "TEST.PA",
                "official_benchmark": "MSCI World",
                "category": "Actions Monde",
                "geo_exposure": "World",
                "region_domicile": "Europe",
                "provider": "Example",
                "pea_type": "PEA",
                "pea_confidence": "HIGH",
