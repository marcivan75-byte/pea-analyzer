...
def test_ci_etf_overlap_keeps_highest_weighted_score():
    dates = pd.date_range("2025-01-01", periods=130, freq="B")
    base = pd.Series(range(100, 230), index=dates, dtype=float)
    selected = pd.DataFrame([
        {"isin": "HIGH", "asset_class": "ETF", "yahoo_ticker": "HIGH.PA", "score": 85, "CI_CONFIDENCE_SCORE_V22_2_1": 70, "CI_TRADINGVIEW_SIGNAL": "BUY", "morningstar_rating": 4, "CI_SELECTION_GATE_STATUS_V4": "SELECTED", "CI_SELECTION_GATE_REASON_V4": "PASS_ALL_SELECTION_GATES"},
        {"isin": "LOW", "asset_class": "ETF", "yahoo_ticker": "LOW.PA", "score": 80, "CI_CONFIDENCE_SCORE_V22_2_1": 90, "CI_TRADINGVIEW_SIGNAL": "STRONG_BUY", "morningstar_rating": 5, "CI_SELECTION_GATE_STATUS_V4": "SELECTED", "CI_SELECTION_GATE_REASON_V4": "PASS_ALL_SELECTION_GATES"},
    ])
    kept, removed, decisions = gate._apply_etf_overlap_gate(
        selected, {"HIGH.PA": base, "LOW.PA": base * 2}
    )
    assert kept["isin"].tolist() == ["HIGH"]
    assert removed["isin"].tolist() == ["LOW"]
