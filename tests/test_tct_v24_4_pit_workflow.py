...
def test_pit_validator_runs_after_snapshot_and_is_artifacted():
    ...
    assert "Run one next-session catalyst snapshot V24.4.1" in workflow
    assert "Validate accumulated V24.4.1 PIT ledger" in workflow
    assert "python -m v182.reporting.tct_v24_4_1_pit_validator_runtime" in workflow
    assert "TCT_V24_4_1_PIT_VALIDATION.json" in workflow
    assert "ANDROID_TCT_V24_4_1_PIT_VALIDATION.md" in workflow
    assert "TCT_V24_4_1_PIT_SLICES.csv" in workflow
    assert "TCT_V24_4_1_PREOPEN_POSTMARKET_CHANGES.csv" in workflow
...
def test_validator_has_no_production_authority():
    ...
    wrapper = (ROOT / "src" / "v182" / "reporting" / "tct_v24_4_1_pit_validator_runtime.py").read_text(encoding="utf-8")
    ...
    assert "TCT_V24_4_1_VALIDATION_GATES.json" in wrapper
    lower = runtime.lower()
