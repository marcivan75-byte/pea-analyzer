...
def build_trade_paths(x):
    rows=[]
    by={t:z.reset_index(drop=True) for t,z in x.groupby('ticker',sort=False)}
    for t,z in by.items():
        idx=np.flatnonzero(z.signal.to_numpy(bool))
        for i in idx:
            ...
            for k,(_,b) in enumerate(bars.iterrows(),1):
                row[f'd{k}']=b.date; row[f'h{k}']=b.high; row[f'l{k}']=b.low; row[f'c{k}']=b.close
            rows.append(row)
...
def exit_price(r,cfg):
    ...
    for d in range(1,6):
        h=float(r[f'h{d}']); l=float(r[f'l{d}']); date=r[f'd{d}']
        s=0.0 if (cfg=='DYN_BE_TP20' and armed) else stop
...
def trade_table(paths,cfg,bps):
    ...
    for _,r in paths.iterrows():
        ...
        rows.append({**r.to_dict(),'config':cfg,'friction_bps_oneway':bps,'exit_date':xd,'exit_px_gross':xp,'exit_px_net':xp_net,'exit_reason':why,'net_ret':ret})
...
def portfolio(trades,max_pos=5,initial=100000.0):
    ...
    dates=sorted(set(t.entry_date)|set(t.exit_date))
    entries={d:z for d,z in t.groupby('entry_date')}; exits={d:z for d,z in t.groupby('exit_date')}
    for d in dates:
        # exits first: capital liberated can be reused same day; conservative enough because exits are intraday/close and entries open, but only rare same-day overlap. Mark for transparency.
        exiting=[p for p in openpos if p['exit_date']==d]
        for p in exiting:
...
def annual_from_accepted(a,initial=100000.0):
    if a.empty: return pd.DataFrame()
    a=a.copy(); a['year']=pd.to_datetime(a.exit_date).dt.year
    rows=[]
    for y,z in a.groupby('year'):
        pnl=float(z.pnl_eur.sum()); rows.append({'year':int(y),'trades':len(z),'wins':int((z.net_ret>0).sum()),'win_rate_pct':100*(z.net_ret>0).mean(),'net_pnl_eur':pnl,'sum_notional_eur':float(z.notional.sum()),'return_on_initial_pct':100*pnl/initial,'mean_trade_pct':100*z.net_ret.mean(),'median_trade_pct':100*z.net_ret.median(),'profit_factor':float(z.loc[z.pnl_eur>0,'pnl_eur'].sum()/(-z.loc[z.pnl_eur<0,'pnl_eur'].sum())) if (z.pnl_eur<0).any() else np.inf})
    return pd.DataFrame(rows)
...
def main():
    ...
    summaries=[]; all_annual=[]; oos_detail=[]
    for cfg in cfgs:
        for bps in bps_grid:
            ...
            for maxp in max_grid:
                for label,lo,hi in [('PRE',2010,2022),('OOS',2023,2026)]:
                    ...
                    summaries.append({'config':cfg,'bps_oneway':bps,'max_positions':maxp,'period':label,'signals':len(q),'accepted':len(a),'skipped_capacity':skipped,'final_equity_eur':final,'portfolio_return_pct':ret,'mean_trade_pct':100*a.net_ret.mean() if len(a) else np.nan,'win_rate_pct':100*(a.net_ret>0).mean() if len(a) else np.nan,'median_capacity_pct_adv20':a.capacity_pct_adv20.median() if len(a) else np.nan,'max_capacity_pct_adv20':a.capacity_pct_adv20.max() if len(a) else np.nan})
                    if label=='OOS' and bps==25 and maxp==5:
                        ann=annual_from_accepted(a); ann['config']=cfg; ann['bps_oneway']=bps; ann['max_positions']=maxp; all_annual.append(ann); oos_detail.append(a.assign(config=cfg))
    s=pd.DataFrame(summaries); s.to_csv(OUT/'PORTFOLIO_GRID.csv',index=False)
    pd.concat(all_annual,ignore_index=True).to_csv(OUT/'OOS_ANNUAL_DEFAULT_25BPS_MAX5.csv',index=False)
    pd.concat(oos_detail,ignore_index=True).to_csv(OUT/'OOS_ACCEPTED_TRADES_DEFAULT.csv',index=False)
    focus=s[(s.period=='OOS')&(s.bps_oneway==25)&(s.max_positions==5)].sort_values('portfolio_return_pct',ascending=False)
    focus.to_csv(OUT/'OOS_DEFAULT_CONFIG_COMPARISON.csv',index=False)
    meta={'pattern':PATTERN,'entry':'Open J+1 after signal known at Close J','portfolio_initial_eur':100000,'default_max_positions':5,'default_all_in_friction_bps_oneway':25,'friction_note':'all-in slippage+fees proxy; excludes security-specific taxes such as French FTT','capacity':'position notional / trailing ADV20 EUR','dynamic_exit':'SL10; after any session reaches +10%, stop moves to breakeven from next session; TP20; time exit day5; same-day stop/target collision=STOP_FIRST','oos_not_used_for_pattern_or_exit_selection':True,'signals_total':len(paths),'focus':focus.to_dict('records')}
